 ...
 */
@Entity
@Table(name = "PROC_RFP_EVENT_BQ")
public class RfpEventBq extends Bq implements Serializable {

	private static final long serialVersionUID = -8425585426647163092L;

	@JsonIgnore
	@ManyToOne(fetch = FetchType.LAZY, optional = true)
	@JoinColumn(name = "EVENT_ID", foreignKey = @ForeignKey(name = "FK_RFP_EVENT_BQ"))
	private RfpEvent rfxEvent;

	@JsonIgnore
	@OneToMany(fetch = FetchType.LAZY, mappedBy = "bq", orphanRemoval = true)
	@OrderBy("level, order")
	private List<RfpBqItem> bqItems;

	@JsonIgnore
	@OneToMany(fetch = FetchType.LAZY, mappedBy = "bq", orphanRemoval = true, cascade = CascadeType.ALL)
	private List<RfpBqTotalEvaluationComments> evaluationComments;

	public RfpEventBq() {

	}

	public RfpEventBq(SourcingFormRequestBq bq) {
	...
	public RfpEventBq createMobileShallowCopy() {
		...
		if (CollectionUtil.isNotEmpty(getBqItems())) {
			...
			for (RfpBqItem bqItem : getBqItems()) {
	...
	public RfpEventBq copyForRfp(boolean isSupplierInvited) {

		RfpEventBq newBq = new RfpEventBq();
		if (isSupplierInvited)
			newBq.setId(this.getId());
		if (CollectionUtil.isNotEmpty(getBqItems())) {
	...
	public RfpEventBq createMobileEnvelopShallowCopy() {
		RfpEventBq bq = new RfpEventBq();
		bq.setId(getId());
		bq.setName(getName());
		bq.setBqOrder(getBqOrder());
	 ...
	 * @return the bqItems
	 */
	public List<RfpBqItem> getBqItems() {
		return bqItems;
	}
	 ...
	 */
	public void setBqItems(List<RfpBqItem> bqItems) {
		if (this.bqItems == null) {
			this.bqItems = new ArrayList<RfpBqItem>();
		} else {
			this.bqItems.clear();
		}
		if (bqItems != null) {
	 ...
	 */
	public List<RfpBqTotalEvaluationComments> getEvaluationComments() {
	 ...
	 */
	public void setEvaluationComments(List<RfpBqTotalEvaluationComments> evaluationComments) {
	...
	public String toLogString() {
		return "RfpEventBillOfQuantity [ " + super.toLogString() + "]";
	}
