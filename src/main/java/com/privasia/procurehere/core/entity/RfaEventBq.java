 ...
 */
@Entity
@Table(name = "PROC_RFA_EVENT_BQ")
public class RfaEventBq extends Bq implements Serializable {

	private static final long serialVersionUID = 6546970334528653718L;

	@JsonIgnore
	@ManyToOne(fetch = FetchType.LAZY, optional = true)
	@JoinColumn(name = "EVENT_ID", foreignKey = @ForeignKey(name = "FK_RFA_EVENT_BQ"))
	private RfaEvent rfxEvent;

	@JsonIgnore
	@OneToMany(fetch = FetchType.LAZY, mappedBy = "bq", orphanRemoval = true)
	@OrderBy("level, order")
	private List<RfaBqItem> bqItems;

	@JsonIgnore
	@OneToMany(fetch = FetchType.LAZY, mappedBy = "bq", orphanRemoval = true, cascade = CascadeType.ALL)
	private List<RfaBqTotalEvaluationComments> evaluationComments;

	public RfaEventBq(SourcingFormRequestBq bq) {
	...
	public RfaEventBq createMobileShallowCopy() {
		...
		if (CollectionUtil.isNotEmpty(getBqItems())) {
			...
			for (RfaBqItem bqItem : getBqItems()) {
	...
	public RfaEventBq createMobileEnvelopShallowCopy() {
		RfaEventBq bq = new RfaEventBq();
		bq.setId(getId());
		bq.setName(getName());
		bq.setBqOrder(getBqOrder());
	 ...
	 * @return the bqItems
	 */
	public List<RfaBqItem> getBqItems() {
		return bqItems;
	}
	 ...
	 */
	public void setBqItems(List<RfaBqItem> bqItems) {
		if (this.bqItems == null) {
			this.bqItems = new ArrayList<RfaBqItem>();
		} else {
			this.bqItems.clear();
		}
		if (bqItems != null) {
	 ...
	 */
	public List<RfaBqTotalEvaluationComments> getEvaluationComments() {
	 ...
	 */
	public void setEvaluationComments(List<RfaBqTotalEvaluationComments> evaluationComments) {
	...
	public String toLogString() {
		return "RfaEventBillOfQuantity [ " + super.toLogString() + "]";
	}
