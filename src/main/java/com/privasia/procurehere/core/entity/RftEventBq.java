 ...
 */
@Entity
@Table(name = "PROC_RFT_EVENT_BQ")
public class RftEventBq extends Bq implements Serializable {
	private static final long serialVersionUID = 1222596171204230219L;

	@JsonIgnore
	@ManyToOne(fetch = FetchType.LAZY, optional = true)
	@JoinColumn(name = "EVENT_ID", foreignKey = @ForeignKey(name = "FK_RFT_EVENT_BQ"))
	private RftEvent rfxEvent;

	@JsonIgnore
	@OneToMany(fetch = FetchType.LAZY, mappedBy = "bq", orphanRemoval = true)
	@OrderBy("level, order")
	private List<RftBqItem> bqItems;

	@JsonIgnore
	@OneToMany(fetch = FetchType.LAZY, mappedBy = "bq", orphanRemoval = true, cascade = CascadeType.ALL)
	private List<RftBqTotalEvaluationComments> evaluationComments;

	public RftEventBq(SourcingFormRequestBq bq) {
	...
	public RftEventBq createMobileShallowCopy() {
		...
		if (CollectionUtil.isNotEmpty(getBqItems())) {
			...
			for (RftBqItem rftBqItem : getBqItems()) {
				bq.getBqItem().add(rftBqItem.createMobileShallowCopy());
			}
		}
	...
	public RftEventBq copyForEnvelop() {
	...
	public RftEventBq copyForRft(boolean isSupplierInvited) {
		RftEventBq newBq = new RftEventBq();
		if (CollectionUtil.isNotEmpty(getBqItems())) {
			newBq.setBqItems(new ArrayList<RftBqItem>());
			for (RftBqItem bqItem : getBqItems()) {
				RftBqItem newBqItem = bqItem.copyForRft(newBq);
				if (isSupplierInvited)
					newBqItem.setId(bqItem.getId());
	...
	public RfqEventBq copyForRfq(boolean invitedSuppliersOnly) {
	...
	public RftEventBq createMobileEnvelopShallowCopy() {
		RftEventBq bq = new RftEventBq();
		bq.setId(getId());
		bq.setName(getName());
		bq.setBqOrder(getBqOrder());
	 ...
	 * @return the bqItems
	 */
	public List<RftBqItem> getBqItems() {
		return bqItems;
	}
	 ...
	 */
	public void setBqItems(List<RftBqItem> bqItems) {
		if (this.bqItems == null) {
			this.bqItems = new ArrayList<RftBqItem>();
		} else {
			this.bqItems.clear();
		}
		if (bqItems != null) {
	 ...
	 */
	public List<RftBqTotalEvaluationComments> getEvaluationComments() {
	 ...
	 */
	public void setEvaluationComments(List<RftBqTotalEvaluationComments> evaluationComments) {
	...
	public String toLogString() {
		return "RftEventBillOfQuantity [ " + super.toLogString() + "]";
	}
}
