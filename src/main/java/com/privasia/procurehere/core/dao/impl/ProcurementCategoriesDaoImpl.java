 ...
 */
@Repository
public class ProcurementCategoriesDaoImpl extends GenericDaoImpl<ProcurementCategories, String> implements ProcurementCategoriesDao {

	private static final Logger LOG = LogManager.getLogger(ProcurementCategoriesDaoImpl.class);

	@SuppressWarnings("unchecked")
	@Override
	public List<ProcurementCategories> findProcurementCategoriesForTenant(String tenantId, TableDataInput input) {
		final Query query = constructProcurementCategoriesTenantQuery(tenantId, input, false);
		query.setFirstResult(input.getStart());
		query.setMaxResults(input.getLength());
		List<ProcurementCategories> pcList = query.getResultList();
	...
	public long findTotalFilteredProcurementCategoriesForTenant(String tenantId, TableDataInput input) {
		final Query query = constructProcurementCategoriesTenantQuery(tenantId, input, true);
		return ((Number) query.getSingleResult()).longValue();
	}

	private Query constructProcurementCategoriesTenantQuery(String tenantId, TableDataInput tableParams, boolean isCount) {
		String hql = "";
		if (isCount) {
			hql += "select count(pm) ";
		}
		hql += " from ProcurementCategories pm ";
	...
	public long findCountOfProcurementCategoriesForTenant(String tenantId) {
		StringBuilder hql = new StringBuilder("select count (pm) from ProcurementCategories pm where pm.status =:status and pm.buyer.id = :tenantId ");
		final Query query = getEntityManager().createQuery(hql.toString());
		query.setParameter("status", Status.ACTIVE);
		query.setParameter("tenantId", tenantId);
		return ((Number) query.getSingleResult()).longValue();
	...
	public boolean isExists(ProcurementCategories procurementCategories, String tenantId) {
		StringBuilder hsql = new StringBuilder("from ProcurementCategories as pm where upper(pm.procurementCategories) = upper(:procurementCategories) and pm.buyer.id = :tenantId ");
		if (StringUtils.checkString(procurementCategories.getId()).length() > 0) {
			hsql.append(" and pm.id <> :id");
		}
		final Query query = getEntityManager().createQuery(hsql.toString());
		query.setParameter("procurementCategories", procurementCategories.getProcurementCategories());
		query.setParameter("tenantId", tenantId);
		if (StringUtils.checkString(procurementCategories.getId()).length() > 0) {
			query.setParameter("id", procurementCategories.getId());
		}
		List<ProcurementCategories> stList = query.getResultList();
		return CollectionUtil.isNotEmpty(stList);
	}

	@SuppressWarnings("unchecked")
	@Override
	public List<ProcurementCategories> getAllProcurementCategoriesByTenantId(String tenantId) {
		final Query query = getEntityManager().createQuery("from ProcurementCategories pm where pm.buyer.id = :tenantId order by pm.procurementCategories");
	...
	public List<ProcurementCategories> getAllActiveProcurementCategory(String tenantId) {
		final Query query = getEntityManager().createQuery("select distinct new com.privasia.procurehere.core.entity.ProcurementCategories(p.id, p.procurementCategories, p.description) from ProcurementCategories p where p.status = :status and p.buyer.id = :tenantId");
		query.setParameter("status", Status.ACTIVE);
		query.setParameter("tenantId", tenantId);
		return query.getResultList();
	}
