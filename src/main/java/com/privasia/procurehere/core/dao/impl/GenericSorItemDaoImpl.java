...
import com.privasia.procurehere.core.dao.GenericSorItemDao;
...
public class GenericSorItemDaoImpl<T extends SorItem, PK extends Serializable> extends GenericDaoImpl<T, String> implements GenericSorItemDao<T, String> {

    private static final Logger LOG = LogManager.getLogger(GenericDaoImpl.class);
    @Override
    public List<T> getSorItemForSearchFilter(String bqId, Integer itemLevel, Integer itemOrder, String searchVal, Integer start, Integer length) {
        LOG.info(" id :" + bqId + "  itemLevel  : " + itemLevel + " itemOrder : " + itemOrder + " searchVal :" + searchVal + " start :" + start + " length :" + length);
        try {
            StringBuilder hql = new StringBuilder("select distinct a from " + entityClass.getName() + " a inner join fetch a.sor bq left outer join fetch a.children c left outer join fetch a.parent p left outer join fetch a.uom uom where bq.id =:id ");
    ...
    public long totalSorItemCountByBqId(String bqId, String searchVal) {
        StringBuilder hql = new StringBuilder("select count(item) from " + entityClass.getName() + " item where item.sor.id =:bqId ");
        if (searchVal != null && StringUtils.checkString(searchVal).length() > 0) {
    ...
    public List<BqItemPojo> getAllLevelOrderSorItemBySorId(String bqId, String searchVal) {
        LOG.info(" sorId  :" + bqId);
        StringBuilder hql = new StringBuilder("select distinct new com.privasia.procurehere.core.pojo.BqItemPojo(a.level, a.order) from " + entityClass.getName() + " a where a.sor.id =:bqId ");
        if (searchVal != null && StringUtils.checkString(searchVal).length() > 0) {
            hql.append(" and (upper(a.itemName) like :searchValue or upper(a.itemDescription) like :searchValue) ");
        }
        hql.append(" order by a.level, a.order ");
    ...
    public boolean isExists(final T bqItem, String bqId, String parentId) {
        ...
        String hql = "select count(*) from " + entityClass.getName() + " b where b.sor.id = :bqId and upper(b.itemName) = :itemName";
    ...
    @Override
    public List<T> getSorItemLevelOrder(String bqId) {
        final Query query = getEntityManager().createQuery("from " + entityClass.getName() + " a inner join fetch a.sor c where a.parent is null and c.id = :id");
        query.setParameter("id", bqId);
    ...
    public void updateItemOrder(final T bqItem, String bqId, String oldParent, String newParent, int oldOrder, int newOrder, int oldLevel, int newLevel) {
        ...
        if (newOrder == 0) {
            ...
            StringBuilder hql = new StringBuilder("update " + entityClass.getName() + " i set i.level = (i.level + 1) where i.sor.id = :bqId");
        ...
        if (oldOrder == 0) {
            ...
            StringBuilder hql = new StringBuilder("update " + entityClass.getName() + " i set i.level = (i.level - 1) where i.level > :level and i.sor.id = :bqId");
            Query query = getEntityManager().createQuery(hql.toString());
            query.setParameter("bqId", bqId);
        ...
        if (newParent != null) {
            ...
            StringBuilder hql = new StringBuilder("update " + entityClass.getName() + " i set i.order = (i.order + 1) where i.parent.id = :parent and i.sor.id = :bqId");
        ...
        if (oldParent != null) {
            ...
            StringBuilder hql = new StringBuilder("update " + entityClass.getName() + " i set i.order = (i.order - 1) where i.parent.id = :parent and i.order > :order and i.sor.id = :bqId");
            Query query = getEntityManager().createQuery(hql.toString());
            query.setParameter("bqId", bqId);
        ...
        if (newParent != null) {
            T newDbParent = getSorItembySorItemId(newParent);
            bqItem.setLevel(newDbParent.getLevel());
        }

        StringBuilder hql = new StringBuilder("update " + entityClass.getName() + " i set i.order = :order, i.level = :level, i.parent.id = :newParent where i.id = :bqItemId and i.sor.id = :bqId");
        Query query = getEntityManager().createQuery(hql.toString());
        query.setParameter("bqId", bqId);
    ...
    public void deleteSorItemsForEventId(String eventId) {
    ...
    @Override
    public List<T> findSorItemsForSor(String sorId) {
        final Query query = getEntityManager().createQuery("from " + entityClass.getName() + " r where r.sor.id =:sorId order by r.level, r.order");
        query.setParameter("sorId", sorId);
        return query.getResultList();
    }

    @Override
    public List<T> getAllSorItemsbysorId(String bqId) {
        final Query query = getEntityManager().createQuery("select distinct a from " + entityClass.getName() + " a inner join fetch a.sor bq left outer join fetch a.children c left outer join fetch a.uom uom where bq.id =:id order by a.level, a.order");
        query.setParameter("id", bqId);
        return query.getResultList();
    }
    ...
    public void deleteSorItemsbySorid(String bqId) {
        Query query = getEntityManager().createQuery("delete from " + entityClass.getName() + " item where item.sor.id = :id and item.parent is not null");
        query.setParameter("id", bqId);
        query.executeUpdate();

        query = getEntityManager().createQuery("delete from " + entityClass.getName() + " item where item.sor.id = :id and item.parent is null");
        query.setParameter("id", bqId);
        query.executeUpdate();
    }
    ...
    public T getParentbyLevelId(String bqId, Integer level) {
        try {
            final Query query = getEntityManager().createQuery("from " + entityClass.getName() + " a inner join fetch a.sor as b where b.id = :bqId and a.level = :level and a.order = 0 ");
            query.setParameter("bqId", bqId);
    ...
    public void resetFieldsForFilledBySupplier(String id, List<String> label) {
        ...
        Query query = getEntityManager().createQuery("update " + entityClass.getName() + " i set " + cols + " where i.sor.id = :id");
    ...
    public void deletefieldInSorItems(String bqId, String label) {
        try {
            ...
            StringBuilder hql = new StringBuilder("update " + entityClass.getName() + " i set i." + label + " = null where i.sor.id = :bqId");
            Query query = getEntityManager().createQuery(hql.toString());
            query.setParameter("bqId", bqId);
            query.executeUpdate();
    ...
    public void deleteChildItems(String parentItemId, String bqId) throws NotAllowedException {
        Query query = getEntityManager().createQuery("select a.level, a.order, p.id from " + entityClass.getName() + " a left outer join a.parent p where a.id = :id");
        ...
        if (CollectionUtil.isNotEmpty(result) && result.get(0).length > 1) {
            ...
            if (StringUtils.checkString(parentId).length() == 0) {
                ...
                // If Child
                StringBuilder hql = new StringBuilder("update " + entityClass.getName() + " i set i.order = (i.order - 1) where i.level = :level and i.order >= :order and i.parent.id = :parentId and i.sor.id = :bqId");
                query = getEntityManager().createQuery(hql.toString());
                query.setParameter("bqId", bqId);
                query.setParameter("level", level);
        ...
        LOG.info("Going to Delete the selected SOR Items");
        ...
        LOG.info("COMPLETED TO DELETE the Selected SOR ITems");
    }

    @Override
    public void deleteRemaingSorItems(String[] bqItemIds, String bqId) throws NotAllowedException {

        Query query = getEntityManager().createQuery("from " + entityClass.getName() + " a inner join fetch a.sor re where a.id in (:id)");
    ...
    @Override
    public T getSorItembySorItemId(String id) {
        try {
            final Query query = getEntityManager().createQuery("from " + entityClass.getName() + " a inner join fetch a.sor c inner join fetch a.rfxEvent re left outer join fetch a.children ch where a.id = :id");
            query.setParameter("id", id);
            List<T> uList = query.getResultList();
            if (CollectionUtil.isNotEmpty(uList)) {
                return uList.get(0);
            ...
            LOG.info("Error while getting Sor Items : " + nr.getMessage(), nr);
            return null;
        }
    }
}