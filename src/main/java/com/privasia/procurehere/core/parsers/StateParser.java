...
/**
 * This parser parses the State List excel file containing the master data
 * 
 * @author Nitin Otageri
 */
public class StateParser extends FileParser<State> {

	private static final Logger LOG = LogManager.getLogger(StateParser.class);

	@Override
	public List<State> parse(File file) throws ExcelParseException {
	 ...
	 */
	private List<State> readContents(Sheet sheet) throws ExcelParseException, IOException {
			...
			if (rowData.length > 0 && rowData[0].getContents().trim().length() != 0) {
				for (int j = 0; j < columnCount; j++) {
					...
					case 1:
						...
						break;
					case 2:
						if (!colTitle.equalsIgnoreCase("Country Code"))
							throw new ExcelParseException("Country List excel file must contain 'Country Code' at column " + (j + 1), "");
						break;
					default:
						break;
					}
			...
			for (int i = startingRow; i < rowCount; i++) {
				...
				if (rowData.length > 0 && rowData[0].getContents().trim().length() != 0) {
					...
					for (int j = 0; j < columnCount; j++) {
						...
						case 0:
							// State Name
							obj.setStateName(cellData);
							break;
						case 1:
							// State Code
							obj.setStateCode(cellData);
							break;
						case 2:
							// Country Code
							Country c = new Country();
							c.setCountryCode(cellData);
							obj.setCountry(c);
							break;
						default:
							break;
