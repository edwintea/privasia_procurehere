 ...
 */
public class ProcurementMehtodFileParser<E extends ProcurementMethod> {

    private static final Logger LOG = LogManager.getLogger(ProcurementMehtodFileParser.class);
    ...
    public ProcurementMehtodFileParser(Class<E> clazz) {
        this.clazz = clazz;
    }
    ...
    public List<E> parse(File file) throws ExcelParseException {
        ...
        try {
            ...
            LOG.info("Procurement Method List Records : " + list.size());
        } catch (Exception e) {
    ...
    private List<E> readContents(Sheet sheet) throws ExcelParseException, IOException {
        ...
        try {
            ...
            if (rowCount > 0) {
                for (startingRow = 0; startingRow < rowCount; startingRow++) {
                    ...
                    if (rowData != null && rowData.getCell(0) != null && rowData.getCell(0).getStringCellValue().length() > 0) {
                        if (rowData.getCell(0).getStringCellValue().trim().equals("PROCUREMENT METHOD CODE")) {
                            headingFound = true;
                            break;
                        }
                    }
            ...
            if (!headingFound) {
                throw new ExcelParseException("Invalid excel file format. Could not find 'PROCUREMENT CODE' as column heading in any row of the file at 1st column to begin data processing.", "");
            }
            ...
            if (columnCount > 0 && rowData.getCell(0).getStringCellValue().trim().length() > 0) {
                for (int j = 0; j < columnCount; j++) {
                    ...
                    if (j < 3 && !colTitle.equalsIgnoreCase(Global. PROCUREMENT_METHOD_EXCEL_COLUMNS[j])) {
                        throw new ExcelParseException("Procurement Method List excel file must contain '" + Global. PROCUREMENT_METHOD_EXCEL_COLUMNS[j] + "' at column " + (j + 1), "");
                    }
                    if (j >= 4) {
                        ...
                        switch (j) {
                            case 0:
                                ...
                                if (StringUtils.checkString(cellData).length() > 0) {
                                    item.setProcurementMethodCode(cellData);
                                } else {
                                    throw new ExcelParseException("Procurement Method is missing row of " + (i + 1), "");
                                }
                                break;
                            case 1:
                                // PROCUREMENT METHOD
                                if (StringUtils.checkString(cellData).length() > 0) {
                                    item.setProcurementMethod(cellData);
                                }
//							else {
//								throw new ExcelParseException("Description is missing row of " + (i + 1), "");
//							}
                                break;
                            case 2:
                                // DESCRIPTION
