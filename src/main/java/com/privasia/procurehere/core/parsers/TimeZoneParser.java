...
public class TimeZoneParser extends FileParser<TimeZone> {
	
	
	private static final Logger LOG = LogManager.getLogger(TimeZoneParser.class);

	@Override
	public List<TimeZone> parse(File file) throws ExcelParseException {
	 ...
	 */
	private List<TimeZone> readContents(Sheet sheet) throws ExcelParseException, IOException {
		...
		try {
			...
			if (rowCount > 0) {
				for (startingRow = 0; startingRow < rowCount; startingRow++) {
					...
					if (rowData.length > 0 && rowData[0].getContents().trim().length() != 0) {
						if (rowData[0].getContents().trim().equals("Timezone")) {
							headingFound = true;
							break;
						}
					}
			...
			if (!headingFound) {
				throw new ExcelParseException("Invalid excel file format. Could not find 'TimeZone' as column heading in any row of the file at 1st column to begin data processing.", "");
			}
			...
			if (rowData.length > 0 && rowData[0].getContents().trim().length() != 0) {
				for (int j = 0; j < columnCount; j++) {
					...
					switch (j) {
					case 0:
						if (!colTitle.equalsIgnoreCase("Timezone"))
							throw new ExcelParseException("TimeZone List excel file must contain 'Timezone' at column " + (j + 1), "");
						break;
					case 1:
						if (!colTitle.equalsIgnoreCase("Description"))
							throw new ExcelParseException("TimeZone List excel file must contain 'Description' at column " + (j + 1), "");
						break;
					default:
						break;
					}
			...
			for (int i = startingRow; i < rowCount; i++) {
				...
				if (rowData.length > 0 && rowData[0].getContents().trim().length() != 0) {
					...
					for (int j = 0; j < columnCount; j++) {
						...
						case 0:
							// Country Name
							obj.setTimeZone(cellData);
							break;
						case 1:
							// Country Code
							obj.setTimeZoneDescription(cellData);
							break;
						default:
							break;
						}
