...
public class SorFileParser<E extends SorItem> {
    private static final Logger LOG = LogManager.getLogger(BqFileParser.class);
    ...
    public SorFileParser(Class<E> clazz) {
        this.clazz = clazz;
    }
    ...
    public List<E> parse(File file) throws ExcelParseException {
        ...
        try {
            ...
            LOG.info("SOR Item List Records : " + list.size());
        } catch (Exception e) {
        ...
        try {
            ...
            if (columnCount > 0 && rowData.getCell(0).getStringCellValue().trim().length() > 0) {
                for (int j = 0; j < columnCount; j++) {
                    ...
                    if (j < 4 && !colTitle.equalsIgnoreCase(Global.SOR_EXCEL_COLUMNS_TYPE_RFX[j])) {
                        throw new ExcelParseException("SOR Item List excel file must contain '" + Global.SOR_EXCEL_COLUMNS_TYPE_RFX[j] + "' at column " + (j + 1), "");
                    }
                    if (j >= 4) {
                        ...
                        for (int j = 0; j < columnCount; j++) {
                            ...
                            switch (j) {
                                ...
                                case 1:
                                        ...
                                        if (item.getOrder() == 0) {
                                            for (String parentItem : parentList) {
                                                if (parentItem.equalsIgnoreCase(cellData))
                                                    throw new ExcelParseException("Parent " + Global.SOR_EXCEL_COLUMNS_TYPE_RFX[1] + " \"" + cellData + "\" " + " is already exists at row number : " + (i + 1), null);
                                            }
                                            parentList.add(cellData);
                                        } else {
                                            for (String childItem : childList) {
                                                if (childItem.equalsIgnoreCase(cellData))
                                                    throw new ExcelParseException("Child " + Global.SOR_EXCEL_COLUMNS_TYPE_RFX[1] + " \"" + cellData + "\" " + " is already exists at row number : " + (i + 1), null);
                                            }
                                            childList.add(cellData);
                                        }
                                        if(StringUtils.checkString(cellData).length() > 250) {
                                            throw new ExcelParseException("SOR Item Name is more than 250 characters at row number : " + (i + 1), null);
                                        }
                                    ...
                                    break;
                                case 2:
                                    if(StringUtils.checkString(cellData).length() > 2000) {
                                        throw new ExcelParseException("Description is more than 2000 characters at row number : " + (i + 1), null);
                                    }
                                    item.setItemDescription(StringUtils.checkString(cellData));
                                    break;
                                case 3:
                                    if (item.getOrder() != 0 && StringUtils.checkString(cellData).length() == 0) {
                                        throw new ExcelParseException("Please provide UOM for at row number : " + (i + 1), null);
                                    }
                                    Uom uom = new Uom();
