...
public class CompanyStatusParser extends FileParser<CompanyStatus> {

	private static final Logger LOG = LogManager.getLogger(CompanyStatusParser.class);

	@Override
	public List<CompanyStatus> parse(File file) throws ExcelParseException {

		List<CompanyStatus> list = null;
		try {
			Sheet sheet = processExcel(file);
			list = readContents(sheet);
			LOG.debug("Company Status List Records : " + list.size());
		} catch (Exception e) {
	 ...
	 */
	private List<CompanyStatus> readContents(Sheet sheet) throws ExcelParseException, IOException {

		List<CompanyStatus> list = new ArrayList<CompanyStatus>();
		Cell[] rowData = null;
		int rowCount;
		int columnCount;
		...
		try {
			...
			if (rowCount > 0) {
				for (startingRow = 0; startingRow < rowCount; startingRow++) {
					...
					if (rowData.length > 0 && rowData[0].getContents().trim().length() != 0) {
						if (rowData[0].getContents().trim().equals("CompanyStatus")) {
							headingFound = true;
							break;
						}
					}
			...
			if (!headingFound) {
				throw new ExcelParseException("Invalid excel file format. Could not find 'CompanyStatus' as column heading in any row of the file at 1st column to begin data processing.", "");
			}
			...
			if (rowData.length > 0 && rowData[0].getContents().trim().length() != 0) {
				for (int j = 0; j < columnCount; j++) {
					...
					switch (j) {
					case 0:
						if (!colTitle.equalsIgnoreCase("CompanyStatus"))
							throw new ExcelParseException("CompanyStatus List excel file must contain 'CompanyStatus' at column " + (j + 1), "");
						break;
					default:
						break;
					}
