 ...
 */
public class BqFileParser<E extends BqItem> {
	...
	public BqFileParser(Class<E> clazz) {
		this.clazz = clazz;
	}
	...
	public List<E> parse(File file) throws ExcelParseException {
		...
		try {
			...
			LOG.info("BQ Item List Records : " + list.size());
		} catch (Exception e) {
		...
		try {
			...
			if (columnCount > 0 && rowData.getCell(0).getStringCellValue().trim().length() > 0) {
				for (int j = 0; j < columnCount; j++) {
					...
					if (j < 7 && !colTitle.equalsIgnoreCase(Global.BQ_EXCEL_COLUMNS_TYPE_1[j])) {
						throw new ExcelParseException("BQ Item List excel file must contain '" + Global.BQ_EXCEL_COLUMNS_TYPE_1[j] + "' at column " + (j + 1), "");
					}
					if (j >= 7) {
									...
									if (item.getOrder() == 0) {
										for (String parentItem : parentList) {
											if (parentItem.equalsIgnoreCase(cellData))
												throw new ExcelParseException("Parent " + Global.BQ_EXCEL_COLUMNS_TYPE_1[1] + " \"" + cellData + "\" " + " is already exists at row number : " + (i + 1), null);
										}
										...
										for (String childItem : childList) {
											if (childItem.equalsIgnoreCase(cellData))
												throw new ExcelParseException("Child " + Global.BQ_EXCEL_COLUMNS_TYPE_1[1] + " \"" + cellData + "\" " + " is already exists at row number : " + (i + 1), null);
										}
