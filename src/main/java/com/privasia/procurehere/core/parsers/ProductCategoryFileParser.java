 ...
 */
public class ProductCategoryFileParser<E extends ProductCategory> {

	private static final Logger LOG = LogManager.getLogger(ProductCategoryFileParser.class);
	...
	public ProductCategoryFileParser(Class<E> clazz) {
		this.clazz = clazz;
	}
	...
	public List<E> parse(File file) throws ExcelParseException {
		...
		try {
			...
			LOG.info("Product Category List Records : " + list.size());
		} catch (Exception e) {
	...
	private List<E> readContents(Sheet sheet) throws ExcelParseException, IOException {
		...
		try {
			...
			if (rowCount > 0) {
				for (startingRow = 0; startingRow < rowCount; startingRow++) {
					...
					if (rowData != null && rowData.getCell(0) != null && rowData.getCell(0).getStringCellValue().length() > 0) {
						if (rowData.getCell(0).getStringCellValue().trim().equals("CATEGORY CODE")) {
							headingFound = true;
							break;
						}
					}
			...
			if (!headingFound) {
				throw new ExcelParseException("Invalid excel file format. Could not find 'CATEGORY CODE' as column heading in any row of the file at 1st column to begin data processing.", "");
			}
			...
			if (columnCount > 0 && rowData.getCell(0).getStringCellValue().trim().length() > 0) {
				for (int j = 0; j < columnCount; j++) {
					...
					if (j < 3 && !colTitle.equalsIgnoreCase(Global.PRODUCT_CATEGORY_EXCEL_COLUMNS[j])) {
						throw new ExcelParseException("PRODUCT CATEGORY List excel file must contain '" + Global.PRODUCT_CATEGORY_EXCEL_COLUMNS[j] + "' at column " + (j + 1), "");
					}
					if (j >= 3) {
					...
					for (int j = 0; j < columnCount; j++) {
						...
						case 0:
							...
							if (StringUtils.checkString(cellData).length() > 0) {
										...
										item.setProductCode(cellData);
							} else {
								throw new ExcelParseException("Category code is missing row of " + (i + 1), "");
							}
							break;
						case 1:
							// PRODUCT NAME 
							if (StringUtils.checkString(cellData).length() > 0) {
								...
								throw new ExcelParseException("Category name is missing row of " + (i + 1), "");
							}
							break;
						case 2:
