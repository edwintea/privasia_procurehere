...
public class RfpSupplierSorItemServiceImpl implements RfpSupplierSorItemService {
    ...
    @Autowired
    RfpSupplierSorDao supplierSorDao;
    ...
    public List<?> getSorItemForSearchFilterForSupplier(String eventBqId, String supplierId, String searchVal, Integer pageNo, Integer pageLength, Integer itemLevel, Integer itemOrder) {
        List<RfpSupplierSorItem> returnList = new ArrayList<RfpSupplierSorItem>();
        Integer start = null;
        if (pageNo != null) {
            start = pageNo - 1;
        }
        ...
        List<RfpSupplierSorItem> bqList = supplierSorItemDao.getSorItemForSearchFilterForSupplier(eventBqId, supplierId, searchVal, start, pageLength, itemLevel, itemOrder);
        if (CollectionUtil.isNotEmpty(bqList)) {
            for (RfpSupplierSorItem item : bqList) {
                RfpSupplierSorItem bqItem = item.createSearchShallowCopy();
    ...
    public List<RfpSupplierSorItem> saveSupplierEventSor(String bqId) {
        List<RfpSupplierSorItem> rftSupplierBqItem = new ArrayList<>();
        List<RfpSorItem> rftEventBq = supplierSorItemDao.getSorItemsbyId(bqId);
        for (RfpSorItem item : rftEventBq) {
            RfpEventSor bq = item.getSor();
            RfpSupplierSor supplierBq = supplierSorDao.findSorByEventIdAndSorName(bq.getRfxEvent().getId(), bq.getId(), SecurityLibrary.getLoggedInUserTenantId());
            if (supplierBq == null) {
                supplierBq = new RfpSupplierSor(item.getSor());
            ...
            RfpSupplierSorItem supplierBqItem = new RfpSupplierSorItem(item, SecurityLibrary.getLoggedInUser().getSupplier(), supplierBq);
            RfpSupplierSorItem obj = supplierSorItemDao.saveOrUpdate(supplierBqItem);
            if (CollectionUtil.isNotEmpty(supplierBqItem.getChildren())) {
                for (RfpSupplierSorItem child : supplierBqItem.getChildren()) {
                    child.setParent(obj);
    ...
    public List<EventEvaluationPojo> getSorEvaluationData(String eventId, String envelopId, List<Supplier> selectedSuppliers, User logedUser, String withOrWithoutTax, Integer itemLevel, Integer itemOrder, String searchVal, Integer start, Integer length) {
        ...
        if (CollectionUtil.isNotEmpty(suppliers)) {
            ...
            for (SorPojo bqObj : bqPojo) {
                ...
                List<RfpSorItem> sorItems = sorItemDao.getSorItemForSearchFilter(bqObj.getId(), itemLevel, itemOrder, searchVal, start, length);

                List<RfpSorItem> sorItemlevelOrders = sorItemDao.getSorItemForSearchFilter(bqObj.getId(), null, null, null, null, null);
                ...
                for (RfpSorItem sorItemLevelOrder : sorItemlevelOrders) {
                    if (CollectionUtil.isNotEmpty(sorItemlevelOrders) && sorItemlevelOrders.size() > sorItemlistSize) {
                ...
                for (RfpSorItem sorItem : sorItems) {
                    ...
                    List<RfpSupplierSorItem> responseList = supplierSorItemDao.findSupplierSorItemsBySorItemIdAndEventId(sorItem.getId(), eventId, suppliers);

                    for (RfpSupplierSorItem item : responseList) {
                        ...
                        if (item.getSorItem().getEvaluationComments() != null && item.getSorItem().getEvaluationComments().size() > 0) {
                            for (RfpSorEvaluationComments com : item.getSorItem().getEvaluationComments()) {
    ...
    @Override
    public List<RfpSupplierSorItem> getAllSupplierSorItemBySorIdAndSupplierId(String bqId, String supplierId) {
        List<RfpSupplierSorItem> returnList = new ArrayList<RfpSupplierSorItem>();
        List<RfpSupplierSorItem> list = supplierSorItemDao.findSupplierSorItemByListBySorIdAndSupplierId(bqId, supplierId);
    ...
    public List<EventEvaluationPojo> getEvaluationDataForSorComparisonReport(String eventId, String envelopId) {
        ...
        if (CollectionUtil.isNotEmpty(qualifiedSuppliers)) {
            ...
            for (SorPojo bq : bqList) {
                ...
                List<RfpSorItem> bqItems = sorItemDao.findSorItemsForSor(bq.getId());
                ...
                for (RfpSorItem bqItem : bqItems) {
                    ...
                    List<RfpSupplierSorItem> responseList = supplierSorItemDao.findSupplierSorItemsBySorItemIdAndEventId(bqItem.getId(), eventId, qualifiedSuppliers);
                    for (RfpSupplierSorItem item : responseList) {
    ...
    private void setBuyerData(List<List<String>> buyerDataList, RfpSorItem bqItem) {
    ...
    private void setBuyerSupplierHeadings(List<String> buyerHeading, List<String> supplierHeading, RfpSorItem bqItem) {
        if (StringUtils.checkString(bqItem.getSor().getField1Label()).length() > 0) {
            if (bqItem.getSor().getField1FilledBy() == BqUserTypes.BUYER) {
    ...
    private void bulidSupplierSorItemList(List<RfpSupplierSorItem> returnList, List<RfpSupplierSorItem> list) {
        if (CollectionUtil.isNotEmpty(list)) {
            for (RfpSupplierSorItem item : list) {
                RfpSupplierSorItem parent = item.createShallowCopy();
                returnList.add(parent);
                if (CollectionUtil.isNotEmpty(item.getChildren())) {
                    for (RfpSupplierSorItem child : item.getChildren()) {
                        if (parent.getChildren() == null) {
                            parent.setChildren(new ArrayList<RfpSupplierSorItem>());
                        }
    ...
    private void setSupplierData(Map<String, List<String>> finalSupplierDataMap, RfpSorItem bqItem, RfpSupplierSorItem item) {
    ...
    @Override
    public List<RfpSupplierSorItem> getAllSupplierSorItemForReportBySorIdAndSupplierId(String sorId, String supplierId) {

        List<RfpSupplierSorItem> list = supplierSorItemDao.findSupplierSorItemByListBySorIdAndSupplierId(sorId, supplierId);
        if (CollectionUtil.isNotEmpty(list)) {
            for (RfpSupplierSorItem item : list) {
    ...
    @Override
    public List<RfpSupplierSorItem> getAllSupplierSorItemForReportBySorIdAndSupplierIdParentIdNotNull(String bqId, String supplierIds) {
