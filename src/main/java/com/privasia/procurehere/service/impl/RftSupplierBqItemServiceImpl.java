...
public class RftSupplierBqItemServiceImpl implements RftSupplierBqItemService {
	...
	@Autowired
	RftBqEvaluationCommentsDao rftBqEvaluationCommentsDao;
	...
	@Override
	@Transactional(readOnly = true)
	public List<RftSupplierBqItem> getAllSupplierBqItemByBqIdAndSupplierId(String bqId, String supplierId) {
		List<RftSupplierBqItem> returnList = new ArrayList<RftSupplierBqItem>();
		List<RftSupplierBqItem> list = rftSupplierBqItemDao.findSupplierBqItemByListByBqIdAndSupplierId(bqId, supplierId);
		LOG.info("ITEM LIST SIZE : " + list.size());
		bulidSupplierBqItemList(returnList, list);
		return returnList;
	}

	private void bulidSupplierBqItemList(List<RftSupplierBqItem> returnList, List<RftSupplierBqItem> list) {
		LOG.info("BUILDING BQ ITEM LIST :: " + list.size());
		if (CollectionUtil.isNotEmpty(list)) {
			for (RftSupplierBqItem item : list) {
				RftSupplierBqItem parent = item.createShallowCopy();
				returnList.add(parent);
				if (CollectionUtil.isNotEmpty(item.getChildren())) {
					LOG.info("BQ CHILDREN :: :: " + item.getChildren().size());
					for (RftSupplierBqItem child : item.getChildren()) {
						if (parent.getChildren() == null) {
							parent.setChildren(new ArrayList<RftSupplierBqItem>());
						}
	...
	public List<RftSupplierBqItem> saveAndGetSupplierEventBq(String bqId, String tenantId) {
		List<RftSupplierBqItem> rftSupplierBqItems = getAllSupplierBqItemByBqIdAndSupplierId(bqId, tenantId);
		if (CollectionUtil.isEmpty(rftSupplierBqItems)) {
			saveSupplierEventBq(bqId, tenantId);
		}

		return rftSupplierBqItems;
	}

	@Override
	@Transactional(readOnly = false, propagation = Propagation.REQUIRES_NEW)
	public void saveSupplierEventBq(String bqId, String tenantId) {
		List<RftBqItem> rftEventBq = rftSupplierBqItemDao.getBqItemsbyId(bqId);
		BigDecimal sum = BigDecimal.ZERO;
		for (RftBqItem item : rftEventBq) {
			LOG.info("ITEM : " + item.getItemName() + " Child : " + item.getChildren().size());
			RftEventBq bq = item.getBq();
			RftSupplierBq supplierBq = rftSupplierBqDao.findBqByEventIdAndSupplierId(bq.getRfxEvent().getId(), bq.getId(), SecurityLibrary.getLoggedInUserTenantId());
			if (supplierBq == null) {
				supplierBq = new RftSupplierBq(item.getBq());
				RftEvent event = eventDao.findByEventId(bq.getRfxEvent().getId());
				supplierBq.setRfxEvent(event);
				supplierBq.setSupplier(SecurityLibrary.getLoggedInUser().getSupplier());
				supplierBq = rftSupplierBqDao.saveOrUpdate(supplierBq);
				LOG.info("supplierBq after insert : " + supplierBq.toLogString());
			}

			RftSupplierBqItem supplierBqItem = new RftSupplierBqItem(item, SecurityLibrary.getLoggedInUser().getSupplier(), supplierBq);
			RftSupplierBqItem obj = rftSupplierBqItemDao.saveOrUpdate(supplierBqItem);
			if (CollectionUtil.isNotEmpty(supplierBqItem.getChildren())) {
				for (RftSupplierBqItem child : supplierBqItem.getChildren()) {
					child.setParent(obj);
					...
					rftSupplierBqItemDao.saveOrUpdate(child);
				}
			}
	...
	@Override
	public List<RftBqItem> getBqItemsbyId(String bqId) {
		return rftSupplierBqItemDao.getBqItemsbyId(bqId);
	}

	@Override
	public RftSupplierBqItem getSupplierBqItemByBqItemAndSupplierId(String itemId, String supplierId) {
		return rftSupplierBqItemDao.getBqItemByBqItemIdAndSuplier(itemId, supplierId);
	}

	@Override
	public RftSupplierBqItem updateSupplierBqItems(RftSupplierBqItem persistObject) {
	...
	public void updateBqItems(List<SupplierBqItem> items, String supplierId) {
		...
		for (SupplierBqItem supplierBqItem : items) {
			...
			RftSupplierBqItem persistObject = getSupplierBqItemByBqItemAndSupplierId(supplierBqItem.getId(), SecurityLibrary.getLoggedInUserTenantId());
		...
		List<RftSupplierBqItem> itemList = getAllSupplierBqItemListByBqIdAndSupplierId(bqId, supplierId);
		for (RftSupplierBqItem supplierBqItem : itemList) {
			LOG.info("PriceType>>>" + supplierBqItem.getPriceType());
			BigDecimal totalAmountWithTax = supplierBqItem.getTotalAmountWithTax();
			if (totalAmountWithTax != null) {
				PricingTypes t = rftSupplierBqItemDao.getPriceTypeByBqItemId(supplierBqItem.getId());
				LOG.info("===================" + t);
				if (totalAmountWithTax != null) {
			...
			/*
			 * RftSupplierBqItem persistObject = getSupplierBqItemByBqItemAndSupplierId(supplierBqItem.getId(),
			 * SecurityLibrary.getLoggedInUserTenantId()); if (persistObject != null) {
			 * persistObject.setUnitPrice(supplierBqItem.getUnitPrice() != null ? supplierBqItem.getUnitPrice() :
			 * BigDecimal.ZERO); persistObject.setTotalAmount(supplierBqItem.getTotalAmount());
			 ...
			 * persistObject.setField9(supplierBqItem.getField9());
			 * persistObject.setField10(supplierBqItem.getField10()); RftSupplierBqItem updatedBqItem =
			 ...
			 * updatedBqItem); } }
			 */
			LOG.info("supplierBqItem AdditionalTax : " + additionalTax + " TaxDescription : " + taxDesc + " BqId :  " + bqId + " supplierId : " + supplierId);
			persistBq = rftSupplierBqService.getSupplierBqByBqAndSupplierId(bqId, supplierId);
			if (persistBq != null) {
				...
				rftSupplierBqService.updateSupplierBq(persistBq);
			}
		}

	}

	private List<RftSupplierBqItem> getAllSupplierBqItemListByBqIdAndSupplierId(String bqId, String supplierId) {
		List<RftSupplierBqItem> list = rftSupplierBqItemDao.findBqItemListAndBqListByBqIdAndSupplierId(bqId, supplierId);
		return list;
	}

	@Override
	@Transactional(readOnly = false)
	public RftSupplierBqItem updateSupplierBqItem(RftSupplierBqItem persistObject) {
		return rftSupplierBqItemDao.update(persistObject);
	}
	...
	public List<EventEvaluationPojo> getBqEvaluationData(String eventId, String envelopId, List<Supplier> selectedSuppliers, User logedUser, String withOrWithoutTax, Integer itemLevel, Integer itemOrder, String searchVal, Integer start, Integer length) {
		...
		if (CollectionUtil.isNotEmpty(suppliers)) {
			...
			for (BqPojo bqObj : bqPojo) {
				...
				// List<RftBqItem> bqItems = bqItemDao.findBqItemsForBq(bqId);
				List<RftBqItem> bqItems = bqItemDao.getBqItemForSearchFilter(bqObj.getId(), itemLevel, itemOrder, searchVal, start, length);

				List<RftBqItem> bqItemlevelOrders = bqItemDao.getBqItemForSearchFilter(bqObj.getId(), null, null, null, null, null);
				...
				for (RftBqItem bqItemLevelOrder : bqItemlevelOrders) {
				...
				for (RftBqItem bqItem : bqItems) {
					...
					List<RftSupplierBqItem> responseList = rftSupplierBqItemDao.findSupplierBqItemsByBqItemIdAndEventId(bqItem.getId(), eventId, suppliers);
					for (RftSupplierBqItem item : responseList) {
						...
						if (item.getBqItem().getEvaluationComments() != null && item.getBqItem().getEvaluationComments().size() > 0) {
							for (RftBqEvaluationComments com : item.getBqItem().getEvaluationComments()) {
					...
					if (j != bqItem.getLevel()) {
						for (Map.Entry<String, BigDecimal> entry : map.entrySet()) {
							...
							if (keyset[1].equals(currantLvl)) {
								...
								List<RftBqEvaluationComments> cmts= rftBqEvaluationCommentsDao.getCommentsByBqIdAndEventId(id,eventId,logedUser);
				...
				// totalAmounts = rftSupplierBqItemDao.findSumOfTotalAmountWithTaxForSupplier(bqId, eventId, suppliers);
				totalAmounts = rftSupplierBqItemDao.findGrandTotalsForEvaluationView(bqObj.getId(), eventId, suppliers);
				...
				for (String total : totalAmounts) {
					if (StringUtils.checkString(total).length() > 0) {
						...
						RftEventBq bq = bqDao.findById(bqObj.getId());
						boolean commentsExist = false;
						if (bq != null && CollectionUtil.isNotEmpty(bq.getEvaluationComments())) {
							for (RftBqTotalEvaluationComments com : bq.getEvaluationComments()) {
				...
				totalAmounts = rftSupplierBqItemDao.findGrandTotalsForView(bqObj.getId(), eventId, suppliers);
				...
				response.setAddtionalTaxInfo(rftSupplierBqItemDao.findAddtionalTaxForView(bqObj.getId(), eventId, suppliers));
	...
	@Override
	public List<EventEvaluationPojo> getBqEvaluationDataForWithOutTax(String eventId, String envelopId, List<Supplier> selectedSuppliers, User logedUser) {
		List<EventEvaluationPojo> returnList = new ArrayList<>();
		List<Supplier> suppliers = null;
		if (CollectionUtil.isEmpty(selectedSuppliers)) {
			suppliers = rftEventSupplierDao.getEventSuppliersForEvaluation(eventId);
			...
			for (String bqId : bqIds) {
				...
				for (RftBqItem bqItem : bqItems) {
					...
					for (RftSupplierBqItem item : responseList) {
						...
						if (item.getBqItem().getEvaluationComments() != null && item.getBqItem().getEvaluationComments().size() > 0) {
							for (RftBqEvaluationComments com : item.getBqItem().getEvaluationComments()) {
								if (com.getCreatedBy().getId().equals(logedUser.getId())) {
									commentsExist = true;
									LOG.info("Comments : " + item.getBqItem().getEvaluationComments().size());
									continue;
								}
				...
				response.setData(data);
				response.setColumns(suppliers);
				List<String> totalAmounts = rftSupplierBqItemDao.findSumOfTotalAmountWithTaxForSupplier(bqId, eventId, suppliers);
				List<BigDecimal> amounts = new ArrayList<BigDecimal>();
				for (String total : totalAmounts) {
					if (StringUtils.checkString(total).length() > 0) {
						BigDecimal value = new BigDecimal(total).setScale((event.getDecimal() != null && !event.getDecimal().isEmpty() ? Integer.parseInt(event.getDecimal()) : 2), BigDecimal.ROUND_DOWN);
						amounts.add(value);
					} else {
				...
				totalAmounts = rftSupplierBqItemDao.findGrandTotalsForView(bqId, eventId, suppliers);
				List<EvaluationTotalAmountPojo> gTotalAmounts = new ArrayList<EvaluationTotalAmountPojo>();
				for (String total : totalAmounts) {
					if (StringUtils.checkString(total).length() > 0) {
						EvaluationTotalAmountPojo pojo = new EvaluationTotalAmountPojo();
						...
						pojo.setBqId(values[1]);
						pojo.setSupplierId(supplierId);
						RftEventBq bq = bqDao.findById(bqId);
						boolean commentsExist = false;
						if (bq != null && CollectionUtil.isNotEmpty(bq.getEvaluationComments())) {
							for (RftBqTotalEvaluationComments com : bq.getEvaluationComments()) {
								if (((com.getCreatedBy().getId().equals(logedUser.getId()) || (envelop.getLeadEvaluater().getId().equals(logedUser.getId()))) && com.getSupplier().getId().equals(supplierId))) {
				...
				// response.setTotalAmounts(amounts);
				response.setEnvelopId(envelopId);
				response.setEventId(eventId);
				returnList.add(response);
	...
	public List<EventEvaluationPojo> getEvaluationDataForBqComparison(String eventId, String envelopId) {
		List<EventEvaluationPojo> returnList = new ArrayList<>();
		List<Supplier> qualifiedSup = rftEventSupplierDao.getEventQualifiedSuppliersForEvaluation(eventId);
		...
		if (CollectionUtil.isNotEmpty(qualifiedSup)) {
			...
			for (String bqId : bqIds) {
				...
				for (RftBqItem bqItem : bqItems) {
					...
					answers.add(bqItem.getUom() != null ? bqItem.getUom().getUom() : "");
					answers.add(bqItem.getQuantity() != null ? String.valueOf(bqItem.getQuantity()) : "");
					List<RftSupplierBqItem> responseList = rftSupplierBqItemDao.findSupplierBqItemsByBqItemIdAndEventId(bqItem.getId(), eventId, qualifiedSup);
					for (RftSupplierBqItem item : responseList) {
						response.setSupplierId(item.getSupplier().getId());
						if (item.getTotalAmountWithTax() != null) {
				...
				List<String> totalAmounts = rftSupplierBqItemDao.findSumOfTotalAmountForSupplier(bqId, eventId, qualifiedSup);
				List<BigDecimal> amounts = new ArrayList<BigDecimal>();
				for (String total : totalAmounts) {
				...
				List<String> gtotal = rftSupplierBqItemDao.findGrandTotals(bqId, eventId, qualifiedSup);
				...
				response.setAddtionalTaxInfo(rftSupplierBqItemDao.findAddtionalTax(bqId, eventId, qualifiedSup));
				response.setTotalAmounts(gTotalAmounts);
				response.setEnvelopId(envelopId);
				response.setEventId(eventId);
	...
	@Override
	public Integer countAllRftSupplierBqItemByEventId(String SupplierId, String eventId) {
		return rftSupplierBqItemDao.getCountOfAllRftSupplierBqItem(SupplierId, eventId);
	}
	...
	public List<?> getBqItemForSearchFilterForSupplier(String eventBqId, String supplierId, String searchVal, Integer pageNo, Integer pageLength, Integer itemLevel, Integer itemOrder) {
		...
		List<RftSupplierBqItem> bqList = rftSupplierBqItemDao.getBqItemForSearchFilterForSupplier(eventBqId, supplierId, searchVal, start, pageLength, itemLevel, itemOrder);
		if (CollectionUtil.isNotEmpty(bqList)) {
			for (RftSupplierBqItem item : bqList) {
				RftSupplierBqItem bqItem = item.createSearchShallowCopy();
	...
	public List<BqItemPojo> getAllLevelOrderBqItemByBqId(String bqId, String supplierId, String searchVal) {
		return rftSupplierBqItemDao.getAllLevelOrderBqItemByBqId(bqId, supplierId, searchVal);
	}
	...
	public List<EventEvaluationPojo> getBqSearchFilterEvaluationData(String eventId, String envelopId, User logedUser, String withOrWithoutTax, Integer itemLevel, Integer itemOrder, String searchVal, Integer start, Integer length, String[] supplierList) {
		...
		for (String bqId : bqIds) {
			...
			// List<RftBqItem> bqItems = bqItemDao.findBqItemsForBq(bqId);
			List<RftBqItem> bqItems = bqItemDao.getBqItemForSearchFilter(bqId, itemLevel, itemOrder, searchVal, start, length);
			Map<String, BigDecimal> map = new LinkedHashMap<String, BigDecimal>();
			Integer j = 1;
			Integer k = 0;
			for (RftBqItem bqItem : bqItems) {
	...
	public long totalBqItemCountByBqId(String bqId, String supplierId, String searchVal) {
		return rftSupplierBqItemDao.totalBqItemCountByBqId(bqId, supplierId, searchVal);
	}
	...
	public List<EventEvaluationPojo> getEvaluationDataForBqComparisonReport(String eventId, String envelopId) {
		...
		if (CollectionUtil.isNotEmpty(qualifiedSuppliers)) {
			...
			for (BqPojo bq : bqList) {
				EventEvaluationPojo response = new EventEvaluationPojo();
				List<List<String>> data = new ArrayList<List<String>>();
				List<RftBqItem> bqItems = bqItemDao.findBqItemsForBq(bq.getId());
				...
				for (RftBqItem bqItem : bqItems) {
					...
					List<RftSupplierBqItem> responseList = rftSupplierBqItemDao.findSupplierBqItemsByBqItemIdAndEventId(bqItem.getId(), eventId, qualifiedSuppliers);
					...
					List<String> totalAmounts = rftSupplierBqItemDao.findSumOfTotalAmountForSupplier(bq.getId(), eventId, qualifiedSuppliers);
					List<BigDecimal> amounts = new ArrayList<BigDecimal>();
					for (String total : totalAmounts) {
						BigDecimal value = null;
						if (StringUtils.checkString(total).length() > 0)
					...
					List<String> gtotal = rftSupplierBqItemDao.findGrandTotals(bq.getId(), eventId, qualifiedSuppliers);
					List<BigDecimal> grandAmounts = new ArrayList<BigDecimal>();
					List<EvaluationTotalAmountPojo> gTotalAmounts = new ArrayList<EvaluationTotalAmountPojo>();
					for (String total : gtotal) {
						BigDecimal value = null;
					...
					response.setGrandTotals(grandAmounts);
					response.setAddtionalTaxInfo(rftSupplierBqItemDao.findAddtionalTax(bq.getId(), eventId, qualifiedSuppliers));
	...
	private void setSupplierData(Map<String, List<String>> finalSupplierDataMap, RftBqItem bqItem, RftSupplierBqItem item) {
	...
	private void setBuyerData(List<List<String>> buyerDataList, RftBqItem bqItem) {
	...
	private void setBuyerSupplierHeadings(List<String> buyerHeading, List<String> supplierHeading, RftBqItem bqItem) {
		if (StringUtils.checkString(bqItem.getBq().getField1Label()).length() > 0) {
			if (bqItem.getBq().getField1FilledBy() == BqUserTypes.BUYER) {
	...
	@Override
	public RftSupplierBqItem getBqItemByRftBqItemId(String itemId, String supplierId) {
		return rftSupplierBqItemDao.getBqItemByRftBqItemId(itemId, supplierId);
	}

	@Override
	public List<RftSupplierBqItem> getAllSupplierBqItemByBqIdAndSupplierIds(String bqId, String supplierIds) {
		return rftSupplierBqItemDao.findSupplierBqItemListByBqIdAndSupplierIds(bqId, supplierIds);
	}

	@Override
	@Transactional(readOnly = true)
	public List<RftSupplierBqItem> getAllSupplierBqItemForReportByBqIdAndSupplierId(String bqId, String supplierId) {
	...
	private void addSupplierSectionTotal(Map<String, BigDecimal> map, RftBqItem bqItem, List<String> sectionTotalRow) {
		if (map != null) {
			for (Map.Entry<String, BigDecimal> entry : map.entrySet()) {
				String[] keyset = entry.getKey().split("-");
				String currantLvl = "" + (bqItem.getLevel() - 1);
	...
	private void calclulateSectionAmount(Map<String, BigDecimal> map, RftSupplierBqItem item, BigDecimal amount) {
		BigDecimal sectionSum = BigDecimal.ZERO;
		if (map.containsKey(item.getSupplier().getId() + "-" + item.getLevel())) {
			sectionSum = map.get(item.getSupplier().getId() + "-" + item.getLevel()).add(amount);
		} else {
	...
	public RfxSupplierBqItemAndRfxEvent getTotalRftSupplierBqItem(String eventId) {
		...
		List<Supplier> suppliers = rftEventSupplierService.getEventQualifiedSuppliersForEvaluation(eventId);
		...
		if (rftEventAward == null) {
			...
			rfxSupplierBqItemAndRfxEvent.setRftEventAward(awd);
		} else {
		...
		rfxSupplierBqItemAndRfxEvent.setRftEventAward(rfxSupplierBqItemAndRfxEvent.getRftEventAward() == null ?
				rftEventAward : rfxSupplierBqItemAndRfxEvent.getRftEventAward());

		rfxSupplierBqItemAndRfxEvent.setRftSupplierBqItemList(supplierBq.getSupplierBqItems());
