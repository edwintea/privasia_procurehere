...
public class OwnerAuditTrailServiceImpl implements OwnerAuditTrailService {

	final Logger LOG = LogManager.getLogger(OwnerAuditTrailServiceImpl.class);
	...
	@Override
	public List<OwnerAuditTrail> findOwnerAuditTrailForTenant(String tenantId, TableDataInput input, Date startDate, Date endDate, String moduleType) {
		List<OwnerAuditTrail> list = ownerAuditTrailDao.findOwnerAuditTrailForTenant(tenantId, input, startDate, endDate, moduleType);
		for (OwnerAuditTrail ownerAuditTrail : list) {
			if (ownerAuditTrail.getModuleType() != null) {
				ownerAuditTrail.setModuleTypeName(ownerAuditTrail.getModuleType().getValue());
			}
		}
		return list;
	}
	...
	@Override
	public long findTotalFilteredOwnerAuditTrailForTenant(String loggedInUserTenantId, TableDataInput input, Date startDate, Date endDate, String moduleType) {
		return ownerAuditTrailDao.findTotalFilteredOwnerAuditTrailForTenant(loggedInUserTenantId, input, startDate, endDate, moduleType);
	}

	@Override
	public long findTotalOwnerAuditTrailForTenant(String loggedInUserTenantId) {
		return ownerAuditTrailDao.findTotalOwnerAuditTrailForTenant(loggedInUserTenantId);
	}
	...
	public JasperPrint getAuditTrailPdf(String loggedInUserTenantId, TableDataInput input, Date startDate, Date endDate, HttpSession session, String moduleType) {
		...
		try {
			...
			List<OwnerAuditTrail> auditTrail = findOwnerAuditTrailForTenant(loggedInUserTenantId, input, startDate, endDate, moduleType);
			if (CollectionUtil.isNotEmpty(auditTrail)) {
				for (OwnerAuditTrail item : auditTrail) {
					...
					if (startDate != null && endDate != null) {
						audit.setTitle("Owner Audit Report from " + sdf.format(startDate) + " to " + sdf.format(endDate));
					}
	...
	@Override
	public void save(OwnerAuditTrail ownerAudit) {
		ownerAuditTrailDao.save(ownerAudit);
