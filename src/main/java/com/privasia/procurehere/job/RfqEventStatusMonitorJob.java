 ...
 */
@Component
public class RfqEventStatusMonitorJob implements BaseSchedulerJob {

	private static final Logger LOG = LogManager.getLogger(RfqEventStatusMonitorJob.class);
	...
	public void execute(JobExecutionContext ctx) {
		LOG.debug("Start  RFQ Event Finish status Job - " + System.currentTimeMillis());
		changeFromActiveToFinish();
		LOG.debug("END  RFQ Event Finish status Job - " + System.currentTimeMillis());
	}

	private void changeFromActiveToFinish() {
		EventStatus status = EventStatus.CLOSED;
		try {
			List<RfqEvent> list = rfqEventDao.getAllActiveEvents();
			Date now = new Date();
			for (RfqEvent event : list) {
				String eventId = event.getId();
				if (event.getEventEnd() != null && now.after(event.getEventEnd())) {
					LOG.info("Changing Event status to CLOSED for RFQ - " + event.getEventName());
