...
public class IdSettingsController {
	...
	@RequestMapping(path = "/idSettingsList")
	public String listId(IdSettings idSettings, Model model) throws JsonProcessingException {
		/*
		 * LOG.info("listIdSettings called"); if(SecurityLibrary.getLoggedInUserTenantId() != null){ List<IdSettings>
		 * eventIdSettingsList = eventIdSettingsService.getAllSettings(SecurityLibrary. getLoggedInUserTenantId());
		 * LOG.info("eventIdSettingsList called:"+eventIdSettingsList);
		 * model.addAttribute("eventIdSettingsList",eventIdSettingsList ); }
		 */
		return "idSettingsList";

	}

	@RequestMapping(path = "/idSettings", method = RequestMethod.GET)
	public ModelAndView idSettingsCreate(@ModelAttribute IdSettings idSettings, Model model) {
		...
		model.addAttribute("idSettingType", IdSettingType.values());
		model.addAttribute("idSettingPattern", IdSettingPattern.values());
		return new ModelAndView("idSettings", "idSettings", new IdSettings());
	}

	@RequestMapping(path = "/idSettings", method = RequestMethod.POST)
	public ModelAndView saveIdSettings(@Valid @ModelAttribute IdSettings idSettings, BindingResult result, Model model, RedirectAttributes redir) {
		...
		/*
		 * if (idSettings.getId() != null) { LOG.info("saveIdSettings Method Called " + idSettings.getId()); }
		 */
		...
		try {
			if (result.hasErrors()) {
				...
				return new ModelAndView("idSettings", "idSettings", idSettings);
			} else {
				...
				if (doValidate(idSettings, SecurityLibrary.getLoggedInUserTenantId())) {
					if (StringUtils.checkString(idSettings.getId()).length() == 0) {
						...
						LOG.info(" *** validation tobe done ***" + fieldValue);
						...
						persistObj.setIdDelimiter(StringUtils.checkString(idSettings.getIdDelimiter()));
						persistObj.setIdDatePattern(StringUtils.checkString(idSettings.getIdDatePattern()));
						...
						persistObj.setEnableSuffix(idSettings.getEnableSuffix());
						
						if (StringUtils.checkString(idSettings.getIdDatePattern()).length() > 0) {
							fieldValue = idSettings.getIdDatePattern();
							LOG.info(" *** validation successes ***" + fieldValue);
		...
		return new ModelAndView("redirect:idSettingsList");
	}

	@RequestMapping(path = "/editIdSettings", method = RequestMethod.GET)
	public ModelAndView editUom(@RequestParam String id, Model model) throws JsonProcessingException {
		...
		LOG.info(idSettings.getIdDelimiter() + "DELIM && PREF" + idSettings.getIdPerfix() + "-------------->" + idSettings.getId() + " .. Tenant id : " + idSettings.getTenantId());
		model.addAttribute("btnValue", messageSource.getMessage("application.update", new Object[] {}, LocaleContextHolder.getLocale()));
		model.addAttribute("idSettingType", IdSettingType.values());
		model.addAttribute("idSettingPattern", IdSettingPattern.values());
		if(Boolean.TRUE == idSettings.getEnableSuffix()) {
			model.addAttribute("checked", "checked");
		}
		return new ModelAndView("idSettings", "idSettings", idSettings);
	}

	@RequestMapping(path = "/idSettingsListData", method = RequestMethod.GET)
	public ResponseEntity<TableData<IdSettings>> idSettingsListData(TableDataInput input) throws JsonProcessingException {
		try {
	...
	private boolean doValidate(IdSettings idSettings, String tenantId) {
		LOG.info(" *** boolean doValidate( idSettings,tenantId) called ***");
	...
	@RequestMapping(value = "/checkUnitCodeEmpty", method = RequestMethod.POST)
	public @ResponseBody ResponseEntity<String> checkRegistrationNumber() {
		HttpHeaders headers = new HttpHeaders();

		Boolean isEmpty = Boolean.FALSE;
		try {
			LOG.info("------------------------------------------------------" + isEmpty);
			isEmpty = eventIdSettingsService.isEmptyUnitCode(SecurityLibrary.getLoggedInUserTenantId());
			if (isEmpty) {
				headers.add("error", "Please add unit code in all business unit's");
				return new ResponseEntity<String>("error", headers, HttpStatus.INTERNAL_SERVER_ERROR);
			}
		} catch (Exception e) {
			LOG.error("Error while checking Unit code empty " + e.getMessage(), e);
			headers.add("error", "Error while checking Unit code");
			return new ResponseEntity<String>("error", headers, HttpStatus.INTERNAL_SERVER_ERROR);
		}
		return new ResponseEntity<String>("success", headers, HttpStatus.OK);
	}

	@RequestMapping(value = "/checkBusinessUnitSetting/{type}", method = RequestMethod.POST)
	public @ResponseBody ResponseEntity<String> checkBusinessUnitSetting(@PathVariable String type) {
		HttpHeaders headers = new HttpHeaders();

		Boolean isBusinessSettingEnable = Boolean.FALSE;
		try {
			LOG.info("------------------------------------------------------" + isBusinessSettingEnable);
			isBusinessSettingEnable = eventIdSettingsService.isBusinessSettingEnable(SecurityLibrary.getLoggedInUserTenantId(), type);
			if (isBusinessSettingEnable) {
				headers.add("error", "Id generation is based on business unit, so you cannot change business unit.");
				return new ResponseEntity<String>("error", headers, HttpStatus.INTERNAL_SERVER_ERROR);
			}
		} catch (Exception e) {
			LOG.error("Error while checking Unit code empty " + e.getMessage(), e);
			headers.add("error", "Error while checking Unit Id Setting");
			return new ResponseEntity<String>("error", headers, HttpStatus.INTERNAL_SERVER_ERROR);
		}
		return new ResponseEntity<String>("success", headers, HttpStatus.OK);
	}

	@RequestMapping(value = "/showPreView", method = RequestMethod.POST)
	public @ResponseBody ResponseEntity<String> showPreView(@RequestParam String type, @RequestParam String basedOn, @RequestParam String length, @RequestParam String pattern, @RequestParam String preFix, @RequestParam String delimiter, @RequestParam String datePattern) {
		HttpHeaders headers = new HttpHeaders();

		LOG.info("=============type:" + type + "=============basedOn:" + basedOn + "=============length:" + length + "=============pattern:" + pattern + "=============preFix:" + preFix + "=============delimiter:" + delimiter + "=============datePattern:" + datePattern);
		...
		try {
			...
			case "BU_DEL_PRE_DEL_DATE_NNNN":
				id = getIdByPatternOrder("BU", delimiter, preFix, delimiter, datePattern, squence);
				break;
			default:
