...
public class RfqInviteSupplierController extends EventInviteSupplierBase {
	...
	public RfqInviteSupplierController() {
		super(RfxTypes.RFQ);
	}
	...
	public String eventDocumentNext(@ModelAttribute("event") RfqEvent event, Model model) {
		event = eventService.getEventById(event.getId());
		if (event != null) {
	...
	public String addSupplier(@PathVariable String eventId, Model model) throws JsonProcessingException {
		List<EventSupplier> eventSupplierList = rfqEventSupplierService.getAllSuppliersByEventId(eventId);
		super.setNullSupplierObject(eventSupplierList);
		RfqEvent event = eventService.loadRfqEventById(eventId);
		model.addAttribute("event", event);
		buildModel(model, eventSupplierList, RfxTypes.RFQ, event);
	...
	public @ResponseBody ResponseEntity<List<EventSupplier>> addCurrentSuppliers(@RequestParam("addSupplier") String addSupplier, @RequestParam("eventId") String eventId, RfqEventSupplier eventSupplier) {
	...
	public String inviteSupplierSaveDraft(RedirectAttributes redir, @RequestParam("eventId") String eventId, Model model) {
		RfqEvent event = eventService.getRfqEventByeventId(eventId);
		model.addAttribute("eventPermissions", eventService.getUserPemissionsForEvent(SecurityLibrary.getLoggedInUser().getId(), eventId));
		...
		try {
			for (String favSupplier : addSupplier) {
				if (rfqEvent.getTemplate() != null && Boolean.TRUE == rfqEvent.getTemplate().getSupplierBasedOnCategory()) {
					favouriteSupplier = favoriteSupplierService.findInvitedSupplierByIndCat(StringUtils.checkString(favSupplier), select_all, supplierSearchPojo, SecurityLibrary.getLoggedInUserTenantId(), rfqEvent.getIndustryCategories(), exclusive, inclusive);
				} else {
				...
				for (FavouriteSupplier favSupp : favSupp1) {
					...
					if (!doValidate(eventSupplier)) {
						eventSupplier.setSubmissionStatus(SubmissionStatusType.INVITED);
						EventSupplier eventFavSupplier = rfqEventSupplierService.saveInvitedSuppliers(eventSupplier);
