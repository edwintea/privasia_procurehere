...
!function( $ ) {
    ...
    Datepicker.prototype = {
        ...
        place: function(){
			...
			elementH =  this.element.height(); //x4
			windowH  = $(document).height() ;// x1 
			elOffset = parseInt(offset.top); //x3 
			pgeYoffset = parseInt(window.pageYOffset); //z1 
			visibleHeight = window.innerHeight; //x2


			topHeight = elOffset - pgeYoffset; //y1
			btoffset = windowH -(visibleHeight+pgeYoffset);//z2

			bottomHeight = windowH -(pgeYoffset+topHeight+elementH+btoffset);
			
			tp = offset.top + this.height;
			if(bottomHeight < 270){
			tp = elOffset-280;
			$(".bsdatepicker").css("top",tp);
			}
