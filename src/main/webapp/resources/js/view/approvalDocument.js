...
$(document).ready(
		function() {
			...
			var files = [];
			$(document).on("change", "#approvalUploadDocument", function(event) {
				files = event.target.files;
			});
			$('.mega').on('scroll', function() {
				$(this).find('.header').css('top', $(this).scrollTop());
			});
			$('#approvalUploadDocument').click(function(e) {
				$('div[id=Load_File-error-dialog]').hide();
			});

			// upload Pr files
			$('#uploadApprovalDoc').click(
					function(e) {
						console.log("uploadApprovalDoc  ::  " );
						e.preventDefault();
						$('div[id=idGlobalError]').hide();
						$('div[id=idGlobalSuccess]').hide();
						if (!$('#approvalDocumentForm').isValid()) {
							$('div[id=Load_File-error-dialog]').show();
							return false;
						}
						if ($('#approvalUploadDocument').val().length == 0) {
						...
						var file_data = $('#approvalUploadDocument').prop('files')[0];
						...
						var uploadUrl = getContextPath() + '/buyer/approvalDocument';
						$.ajax({
							url : uploadUrl,
							data : form_data,
							cache : false,
							...
							success : function(data, textStatus, request) {
								...
								document.getElementById("approvalUploadDocument").value = "";
							...
							//	$('div[id=Load_File-error-dialog]').hide();
								...
								$.each(data, function(i, item) {
									...
									table += '<tr>'+ '<td class="width_200 width_200_fix align-center"><form method="GET">' + '<a id="downloadButton" data-placement="top" title="Download" href="' + getContextPath()
										+ '/buyer/downloadApprovalDocument/' + item.id + '"><img src="' + getContextPath() 
...
$(document).on("change", "#approvalUploadDocument", function() {
...
$(document).delegate(
		...
		function(e) {
			...
			$.ajax({
				type : "POST",
				url : getBuyerContextPath('updateApprovalDocumentDesc'),
				data : {
					docId : docId,
					docDesc : docDesc,
					formId : formId
...
$(document).delegate(
		...
		function(e) {
			...
			var formId = $('#formId').val();
			var removeUrl = getContextPath() + '/buyer/removeApprovalDocument';
			$.ajax({
				url : removeUrl,
				data : ({
