					...
					function(e) {
						...
						$.ajax({
							...
							success : function(data, textStatus, request) {
								...
								document.getElementById("idInternal").checked = true;
								$(".show_name").html('');
								...
								$.each(data, function(i, item) {
									...
									table += '<tr>'+ '<td class="width_100 width_100_fix align-left wo-rp"><form method="GET">' + '<a id="downloadButton" data-placement="top" title="Download" href="' + getContextPath()
										+ '/buyer/po/downloadPoDocument/' + item.id + '"><img src="' + getContextPath() 
										+ '/resources/images/download.png"></a>&nbsp;' + '<span><a href="" class="removeDocFile" data-placement="top" title="Delete" removeDocId="' + item.id
										+ '"><img src="' + getContextPath() + '/resources/images/delete.png"></a></span>&nbsp<a href=""><span class="editDocFile" data-placement="top" title="Edit"  editDocInternal="'+item.internal+'" editDocId="' + item.id + '" editDocDec="' + itemdescription + '"><img src="' 
										+ getContextPath() + '/resources/images/edit1.png"></span></a></form></td>' 
										+ '<td class="width_200 width_200_fix align-left">' + item.fileName + '</td>' + '<td class="width_200 width_200_fix align-left">' 
										+ itemdescription + '</td>'
										+ '<td class="width_100 width_100_fix align-left">' + item.uploadDate + '</td>'
										+ '<td class="width_100 width_100_fix align-left">' + item.fileSizeInKb + 'KB</td>'
										+ '<td class="width_100 width_100_fix align-left">' + (item.internal == true ? "Internal" : "External") + '</td></tr>';
								});
...
$(document).on("change", "#poUploadDocument", function() {
...
$(document).delegate('.editDocFile', 'click', function(e) {
	...
	var editDocInternal = $(this).attr('editDocInternal');
	if(description == 'undefined'){
		description = "";
	}
	if(editDocInternal == 'undefined'){
		editDocInternal = true;
	}
	
	console.log("editDocInternal " + editDocInternal);
	$('#documentDescriptionPopup').find('#docDec').val(description);
	 if(editDocInternal === "true"){
...
$(document).delegate(
		...
		function(e) {
			...
			var poId = $('#poId').val();
			var internal = $('#internal').is(':checked');
			var header = $("meta[name='_csrf_header']").attr("content");
			var token = $("meta[name='_csrf']").attr("content");
			$.ajax({
				type : "POST",
...
$(document).delegate('#confDelDocument','click',
		function(e) {
			e.preventDefault();
			$('div[id=idGlobalError]').hide();
			$('div[id=idGlobalSuccess]').hide();
			...
			$.ajax({
				...
				success : function(data, textStatus, request) {
					...
					$.each(data, function(i, item) {
						...
						table += '<tr>'+ '<td class="width_100 width_100_fix align-left wo-rp><form method="GET">' + '<a id="downloadButton"  href="' + getContextPath()
							+ '/buyer/downloadPoDocument/' + item.id + '"><img src="' + getContextPath() 
							+ '/resources/images/download.png"></a>&nbsp;' + '<span><a href="" class="removeDocFile" removeDocId="' + item.id
							+ '"><img src="' + getContextPath() + '/resources/images/delete.png"></a></span>&nbsp<a href=""><span class="editDocFile" editDocInternal="'+item.internal+'" editDocId="' + item.id + '" editDocDec="' + itemdescription + '"><img src="' 
							+ getContextPath() + '/resources/images/edit1.png"></span></a></form></td>' 
							+ '<td class="width_200 width_200_fix align-left">' + item.fileName + '</td>' + '<td class="width_200 width_200_fix align-left">' 
							+ itemdescription + '</td>'
							+ '<td class="width_100 width_100_fix align-left">' + item.uploadDate + '</td>'
