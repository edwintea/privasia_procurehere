...
$(document).delegate('.role-bottom-ul li [type="radio"]', 'click', function() {
	...
	$.uniform.update();
	$("#sub-credit").slideUp();
	$(this).closest('#sub-credit').parent().removeClass('has-error').children('span.help-block.form-error').remove();
});
...
$(document).ready(function() {
	...
	$('input.approvalCheck[type="checkbox"][title="Read Only"], input.approvalCheck[type="checkbox"][title="Optional"]').change(function() {
		$(this).closest('.row').next().removeClass('has-error').find('span.help-block.form-error').remove();
		if ($(this).prop('checked')) {
			...
			if ((labelVal == '' || $(this).closest('.row').next().next().find('select').length == 0) && $(this).attr('title') == "Read Only") {
				$(this).prop('checked', false);
				$.uniform.update();
				$(this).closest('.row').next().append('<span class="help-block form-error">This is a required field</span>').addClass('has-error');
				return false;
			}
			$(this).closest('.check-wrapper').siblings('.check-wrapper.first').find('input[type="checkbox"]').prop('checked', true);
			$.uniform.update();
