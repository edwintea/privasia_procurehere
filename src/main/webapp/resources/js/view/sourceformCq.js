...
$(document).ready(function () {
	...
	$(document).delegate('.editCq', 'click', function (e) {
		...
		/*
		 * $('#idCreateRftCq').find('#description').val(cqDesc); $('#idCreateRftCq').find('#name').val(cqName);
		 * $('.meeting2-heading').find('h3').text('Update Questionnaire'); $('#cqSave').text('Update');
		 * $(".cq_form_google_form").find('#itemId').val(''); $('.toggleCreateBq').parent().next().slideDown();
...
$('#activePrTemplate').click(function () {
	$('div[id=idGlobalError]').hide();
	$('div[id=idGlobalSuccess]').hide();
	$('.customError').html('');
	var templateTitle = $('#tempTitle').val();
	templateTitle = "Template " + templateTitle;

	$("#prTemplateactivePopup").dialog({
		modal: true,
		minWidth: 300,
		width: '50%',
		maxWidth: 400,
	...
	$('.ui-widget-overlay').addClass('bg-white opacity-60');
	$('#prTemplateactivePopup').find('#tempId').val($('#templateId').val());
	$('.ui-dialog-title').text(templateTitle);
});

$('#saveAsSourcingTemp').click(function () {
	...
	var ajaxUrl = getContextPath() + '/buyer/copySourcingTemplate'
		...
		if (templateDesc.length <= 250) {
			$.ajax({
				...
				success: function (data, textStatus, request) {
					...
					window.location.href = getContextPath() + "/buyer/editSourcingTemplate?id=" + data + "&success=" + info;
...
$("#basecurrencyVisibleId").change(function () {
	var visibleChecked = $("#basecurrencyVisibleId").is(':checked');
	var readOnlyChecked = $("#baseCcurrencyReadOnlyId").is(':checked');
	if (!visibleChecked && !readOnlyChecked) {
		$("#chosenBaseCurrency").attr('data-validation', 'required');
	} else {
		$("#chosenBaseCurrency").removeAttr('data-validation');
	}

});

$("#baseCcurrencyReadOnlyId").change(function () {
	var visibleChecked = $("#basecurrencyVisibleId").is(':checked');
	var readOnlyChecked = $("#baseCcurrencyReadOnlyId").is(':checked');
	if (!visibleChecked && !readOnlyChecked) {
		$("#chosenBaseCurrency").attr('data-validation', 'required');
...
$(document).on("click", ".reArrangeCqOrder", function(e) {
	$('#confirmReArrangeOrder').modal('show');
});

$('#updateTemplateCqOrder').click(function(e) {
	e.preventDefault();
	var cqId = [];
	...
	var token = $("meta[name='_csrf']").attr("content");
	$.ajax({
		url : getContextPath() + '/buyer/updateTemplateCqOrder/' + formId,
		...
		success : function(data, textStatus, request) {
			...
			window.location.href = getContextPath() + "/buyer/sourcingFormCqList/"+formId+"?success="+$.trim(success);
		},
		error : function(request, textStatus, errorThrown) {
