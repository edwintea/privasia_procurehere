...
$(document).ready(function() {
	...
	var ns = $('ol.sortable').nestedSortable({
		...
		stop : function(item, currentItem) {
			...
			$.ajax({
				url : getContextPath() + "/buyer/sourcingBQOrder",
	...
	$(document).delegate('.editBqItem', 'click', function(e) {
		...
		$('.meeting2-heading').find('h3').text('Edit Bill of Quantity');
		$('#bqSave').text('Update');
	});
	...
	$(document).on("click", ".add_question_table", function() {
		...
		var bqId = $('#bqId').val();
		$.ajax({
			url : getContextPath() + "/buyer/getSourcingBqForNewFields",
			data : {
				bqId : bqId
			},
			type : "GET",
	...
	$(document).on("click", ".Edit_section_table", function() {
		...
		$.ajax({
			url : getContextPath() + "/buyer/getSourcingBqForEdit",
			data : {
				bqId : bqId,
				bqItemId : bqItemId
			},
			...
			success : function(data, textStatus, request) {
				...
				$.each(data.bq, function(i, item) {
					...
					if (i == "field1Label" || i == "field2Label" || i == "field3Label" || i == "field4Label" || i == "field5Label" || i == "field6Label" || i == "field7Label" || i == "field8Label" || i == "field9Label"
						...
						additionalHtml += '<input type="text" data-validation="length" data-validation-length="0-100" class="form-control"   name="' + fieldname + '" id="' + id + '"  placeholder="Enter ' + label
								+ '" value="">' + '</div></div>';
					}
				...
				$('.extraFields').html(additionalHtml);
				$.each(data, function(i, item) {
					if (i == 'unitPrice' || i == 'unitBudgetPrice' || i == 'quantity') {
						$("#creat_subitem_form").find('[name="' + i + '"]').val(ReplaceNumberWithCommas(item.toFixed($('#sourcingDecimal').val())));
					} else {
...
$(function() {
	...
	$('#itemSave').click(function(e) {
		...
		var ajaxUrl = getContextPath() + "/buyer/createSourcingBqItem";
		if (bqItemId != '') {
			ajaxUrl = getContextPath() + "/buyer/updateSourcingBqItem";
		}
		$.ajax({
			url : ajaxUrl,
			data : JSON.stringify(data),
...
$('#idConfirmDeleteBQs').click(function() {
	...
	// // return false;
	$.ajax({
		url : getContextPath() + "/buyer/deleteSourcingBqItems/" + bqId,
		data : {
			'items' : items,
			'filterVal' : filterVal,
			'searchVal' : searchVal,
...
$('#bqSave').click(function(e) {
	...
	var token = $("meta[name='_csrf']").attr("content");
	if ($.trim(bqId) != '') {
		urlAjax = getContextPath() + "/buyer/updateSourcingBqList";
	} else {
		urlAjax = getContextPath() + "/buyer/createSourcingRequestBqList";
	}
	$.ajax({
		url : urlAjax,
		data : JSON.stringify(data),
...
function deleteBQ(bqId) {
	...
	data["formId"] = formId;
	$.ajax({
		url : getContextPath() + "/buyer/deleteSourcingBq",
		data : JSON.stringify(data),
		type : "POST",
		contentType : "application/json",
		dataType : 'json',
...
$('.editBqItem').click(function(e) {
	...
	$.ajax({
		type : "POST",
		url : getContextPath() + "/buyer/getSourcingBqbyId",
		data : {
			'formId' : formId,
			'bqId' : bqId
		},
...
function searchFilterBqItem(choosenVal, searchVal, pageNo, pageLength, isPageEnable, isChooseSection) {
	...
	$.ajax({
		type : "POST",
		url : getContextPath() + "/buyer/getSourcingBqItemForSearchFilter",
		data : {
			'formId' : formId,
			'bqId' : bqId,
			'filterVal' : filterVal,
...
function drawSection(item, disableDragDrop, isPageEnable) {
	...
	table += '	<td class="width_200 width_200_fix align-right">&nbsp;</td>';
	table += '	<td class="width_200 width_200_fix align-right">&nbsp;</td>';
	table += '	<td class="width_200 width_200_fix align-right">&nbsp;</td>';
...
function drawItems(item, disableDragDrop, isPageEnable) {
	...
	if (item.quantity != undefined) {
		table += '			<td class="width_200 width_200_fix align-right">' + ReplaceNumberWithCommas(item.quantity.toFixed($('#sourcingDecimal').val())) + '</td>';
	} else {
		table += '			<td class="width_200 width_200_fix align-right"> &nbsp; </td>';
	}
	if (item.unitPrice != undefined) {
		table += '			<td class="width_200 width_200_fix align-right">' + ReplaceNumberWithCommas(item.unitPrice.toFixed($('#sourcingDecimal').val())) + '</td>';
	} else {
		table += '			<td class="width_200 width_200_fix align-right"> &nbsp; </td>';
	}
	if (item.unitBudgetPrice != undefined) {
		table += '			<td class="width_200 width_200_fix align-right">' + ReplaceNumberWithCommas(item.unitBudgetPrice.toFixed($('#sourcingDecimal').val())) + '</td>';
	} else {
		table += '			<td class="width_200 width_200_fix align-right"> &nbsp; </td>';
	}	
	if (item.bq.field1Label != undefined) {
...
$('#downloadTemplate').click(function(e) {
	...
	window.location.href = getBuyerContextPath('bqItemRfsTemplate') + "/" + bqId;
})
...
$("#uploadBqItemFile").on("change", function() {
	...
	// console.log(myForm);
	$.ajax({
		url : getBuyerContextPath('uploadRfsBqItems'),
		data : myForm,
		type : "POST",
...
$('#updateBqOrder').click(function(e) {
	...
	var token = $("meta[name='_csrf']").attr("content");
	$.ajax({
		url : getContextPath() + '/buyer/updateSourcingBqOrder/' + formId,
...
function buildtable(data, formId, token){
	...
	$.each(data, function(i, item) {
		...
		table += '<tr><td class="col-md-3 width_100_fix" data-id="' + item.id + '">' + '<span class="marg-right-5 pull-left">' + '<form action="' + getContextPath() + '/buyer/showSourcingBqItems" method="post">';
		...
		table += '</span><spring:url value="showSourcingBqItems" var="showBqItems" htmlEscape="true" />' + '</td>';
