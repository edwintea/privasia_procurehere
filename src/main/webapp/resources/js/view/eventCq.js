$(document).ready(function() {
	...
	/*
	 * $('#closeCqItemDiv').click(function(e) { e.preventDefault();
	 * 
	 * $("body").focus(); $('.cq_form_google_form').slideUp(); $('#parentId').val(''); $('#itemId').val(''); // $(".main-New-Question").show();
	 * $('div[id=idGlobalError]').hide(); $('div[id=idGlobalSuccess]').hide(); $(".form-error").remove(); $("#itemName").removeClass("error");
	 * $("#itemDesc").removeClass("error"); $("#creat_seaction_form").hide(); $(".main-edit-Question").hide();
	 * $(".main-New-Question").find('#itemId').val(''); $('.toggle-button').removeClass('toggle-button-selected').next().val('0');
	 * $('.cq_form_google_form').find('#itemName, #itemDesc, input[type="text"]').val(''); $('.cqr_title3').css({ 'top' : '10px', 'font-size' : "20px"
	 * }); $('.cqr_title2').css({ 'top' : '10px', 'font-size' : "15px" }); });
	 */
			...
			success : function(data, textStatus, request) {
				...
				$.each(data, function(name, value) {
					...
					if (value.cqType == "DOCUMENT_DOWNLOAD_LINK") {
						//$("#documentListArray").val('4028803c6d1b449a016d1b4729720008').trigger("chosen:updated");
...
function renderGrid(item) {
	...
	table += '</div></td>' //
		+ '<td class="width_50 width_50_fix align-left"><span class="section_name">' + item.level + '.' + item.order + '</span></td>' //
	+ '<td class="width_200 width_200_fix align-left">' + '<span class="section_name">' + item.itemName + '</span>';
...
$('#updateCqOrder').click(function(e) {
	...
	var token = $("meta[name='_csrf']").attr("content");
	$.ajax({
		url : getContextPath() + '/buyer/'+eventType+'/updateEventCqOrder/' + eventId,
		...
		success : function(data, textStatus, request) {
			...
			window.location.href = getContextPath() + "/buyer/"+eventType+"/eventCqList/" + eventId + "?success=" + $.trim(success);
		},
		error : function(request, textStatus, errorThrown) {
