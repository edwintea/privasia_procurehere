...
$(document).ready(function() {
	var ns = $('ol.sortable').nestedSortable({
		...
		stop : function(item, currentItem) {
			...
			$.ajax({
				url : getContextPath() + '/buyer/requestCqOrder',
	...
	$('#idConfirmDelete').click(function() {
		...
		console.log("CqId : " + cqId);
		$.ajax({
			url : getBuyerContextPath('deleteSourcingTemplateCq') + "/" + cqId,
...
$(function() {
	...
	$(document).on("click", "#createCqItem", function(e) {
		...
		var ajaxUrl = getContextPath() + '/buyer/addSourcingFormItem';
		if ($.trim(itemId) != '') {
			var ajaxUrl = getContextPath() + '/buyer/updateSourcingFormCqItem';
		}
		$.ajax({
			url : ajaxUrl,
			data : JSON.stringify(data),
...
function renderGrid(item) {
	...
	var table = '';
	var isTemplateUsed = $('#isTemplateUsed').val();
	...
	if ((isTemplateUsed == "false") && (suspensionType === 'DELETE_NOTIFY' || suspensionType === 'DELETE_NO_NOTIFY' || eventstatus === 'DRAFT' || eventstatus === 'ACTIVE' || eventstatus === 'INACTIVE')) {
		table += '<label> <input type="checkbox" id="inlineCheckbox116" class="custom-checkbox checksubcheckbox" value="' + item.id + '"></label>';
	}
	table += '</div></td>' + '<td class="width_50 width_50_fix align-left"><span class="sectionNameD">' + item.level + '.' + item.order + '</span></td>' + '<td class="width_200 width_200_fix align-left">' + '<span class="sectionNameD">' + item.itemName + '</span>';
	...
	table += '<td class="width_200 width_200_fix align-left">' ;
		if (isTemplateUsed == "false") {
			table += '<button class="btn btn-info ph_btn_extra_small hvr-pop hvr-rectangle-out add_question_table">Add Question</button>';
		}
	table += '</td>' + '<td class="width_200 width_200_fix align-left">';
	if (isTemplateUsed == "false") {
	table += '<a title="" class="btn btn-sm btn-black ph_btn_small color-white hvr-pop hvr-rectangle-out1 edit_section_btn_table" href="#">Edit</a>';
	}
	table += '</td></tr></table>';
	// }
	table += '</div>';
	if (item.children != undefined && item.children != null) {
		...
		$.each(item.children, function(i, child) {
			...
			table += '</label></div>' + '</td><td  class="width_50 width_50_fix align-left itemNameD">' + child.level + '.' + child.order + '</td>' + '<td  class="width_200 width_200_fix align-left">' + '<span class="itemNameD">'
					+ child.itemName + '</span> ' ;
			...
			table += '</td><td  class="width_200 width_200_fix align-left">' + child.cqTypeName + '</td>' + '<td  class="width_200 width_200_fix align-left">';
			if (isTemplateUsed == "false") {
				table += '<a title="" class="btn btn-sm edit-btn-table ph_btn_small color-white hvr-pop hvr-rectangle-out1 edit_btn_table">Edit</a>' ;
			}
	...
	$(".sortable > li").each(function(index) {
		...
		if (isTemplateUsed == "false") {
			if (par > 0) {
				...
				var editbtn = '<a class="btn btn-sm edit-btn-table ph_btn_small color-white hvr-pop hvr-rectangle-out1 edit_sections_btn_table editItemclass" title="">Edit</a>';
				...
				$(this).find('ol li').each(function(index2) {
					...
					var htm = '<a  id = "editItem" class="btn btn-sm edit-btn-table ph_btn_small color-white hvr-pop hvr-rectangle-out1 edit_btn_table editItemclass" title="">Edit</a>';
				...
				var editbtn = '<a class="btn btn-sm edit-btn-table ph_btn_small color-white hvr-pop hvr-rectangle-out1 edit_section_btn_table editItemclass" title="">Edit</a>';
...
$(document).on("click", ".edit_sections_btn_table", function() {
	...
	var header = $("meta[name='_csrf_header']").attr("content");
	var token = $("meta[name='_csrf']").attr("content");
	$('#saveCqSection').text(updateLabel);
	var itemId = $(this).parents('.sub_item').attr('data-id');
	var ajaxUrl = getContextPath() + '/buyer/getCqItemData';
	$.ajax({
		url : ajaxUrl,
