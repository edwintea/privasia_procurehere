...
$(document).ready(function() {
	var decimalLimit = 2; // Allow up to 2 decimal places
    ...
    $(document).delegate('.itemValue', 'change', function(e) {
        ...
        // Update the regex to allow for a leading decimal point
        var regex = new RegExp("^(\\d*\\.?\\d{0," + decimalLimit + "})$");
        var match = $.trim(this.value).replace(/,/g, "").match(regex);

        if (match) {
            var value = parseFloat(match[0]);
            this.value = ReplaceNumberWithCommas(value.toFixed(decimalLimit));
        } else {
            // If the input doesn't match, you can handle it accordingly
	...
	$('#nextButton').click(function(e) {
        ...
        let href=getContextPath() + "/buyer/poRemark/"+$('#poId').val()+"?prId="+$('#prId').val();

		window.location.href=href
        //supplier is not editable should carry from PR so just jump to
		//$("#poSupplierForm").attr("action", getContextPath() + "/buyer/poPurchaseItemNext/"+$('#poId').val()+"?prId="+$('#prId').val());
		//$('#poSupplierForm').submit();
	});

	$('#saveOpenSupplier').click(function(e) {
		$("#poSupplierForm").attr("action", getContextPath() + "/buyer/savePoForOpenSupp");
		$('#poSupplierForm').submit();
	});
	...
	var ns = $('ol.sortable').nestedSortable({
		...
		stop : function(item, currentItem) {
			...
			data["order"] = newPos;
			$.ajax({
				url : getContextPath() + '/buyer/poOrder',
				...
				error : function(request, textStatus, errorThrown) {
				    if(textStatus !=='error'){
                        $('p[id=idGlobalSuccessMessage]').html(itemName+" successfully saved.");
                        $('.alert').hide();
                        $('.alert-danger').hide();
                        $('div[id=idGlobalSuccess]').show();
                        $(window).scrollTop($('div[id=idGlobalSuccess]').offset().top);
                        setTimeout(function(){
                            window.location.reload();
                        },1000)
                    }else{
                        isError=true;
                        $('p[id=idGlobalErrorMessage]').html(itemName+" is already exists");
                        $('.alert').hide();
                        $('.alert-danger').hide();
                        $('div[id=idGlobalError]').show();
					...
					$('p[id=idGlobalSuccessMessage]').html(itemName+" successfully moved");
                    $('.alert').hide();
                    $('.alert-danger').hide();
                    $('div[id=idGlobalSuccess]').show();
                    $('#loading').hide();
	...
	$('#idConfirmResetButtonBQs').click(function() {
		...
		$.ajax({
			...
			success : function(data, textStatus, request) {
				...
				$.each(data, function(i, item) {
					table += renderGrid(item);
					total_bat = item.po.total;
					addTax = item.po.additionalTax;
					gTotal = item.po.grandTotal;
					decimal = item.po.decimal;
				});
	...
	$('#idConfirmDeleteBQs').click(function() {
		...
		$.ajax({
			url : getContextPath() + '/buyer/deletePoItem/' + poId,
			data : JSON.stringify(items),
			type : "POST",
			dataType : 'json',
			beforeSend : function(xhr) {
	...
	// on Edit fill value as item in grid
	//PH-4113
	...
	$(document).on("click", ".Edit_subitme_table", function() {
		...
		var poId = $('#poId').val();
        var prId = $('#prId').val();
		var poItemId = $(this).parents('.sub_item').attr('data-id');
...
$(function() {
	...
	$('#itemSave').click(function(e) {
		...
		var ajaxUrl = getContextPath() + '/buyer/createPoItem';
		if (poItemId != '') {
		...
		$.ajax({
			...
			success : function(data, textStatus, request) {
				...
				if(data.length > 0){
				    $.each(data, function(i, item) {
                        if(item.po.decimal != null && item.po.decimal !==undefined && item.po.total !==undefined){
                            total_bat = ReplaceNumberWithCommas(item.po.total.toFixed(item.po.decimal));
                        }
                        if(item.po.decimal != null && item.po.decimal !==undefined && item.po.additionalTax !==undefined){
                            addTax = ReplaceNumberWithCommas(item.po.additionalTax.toFixed(item.po.decimal));
                        }
                        if(item.po.decimal != null && item.po.decimal !==undefined && item.po.grandTotal !==undefined){
                            gTotal = ReplaceNumberWithCommas(item.po.grandTotal.toFixed(item.po.decimal));
                        }
...
$(function() {
	var header = $("meta[name='_csrf_header']").attr("content");
    var token = $("meta[name='_csrf']").attr("content");
    $(document).on("click", "#savePoSection", function(e) {

        e.preventDefault();
        if ($('#demo-form1').isValid()) {
            var poItemId = document.getElementById("itemId").value;
...
$('#newFieldsSave').click(function(e) {
	...
	$.ajax({
		url : getContextPath() + '/buyer/addNewColumnsPoItem',
		data : JSON.stringify(data),
		type : "POST",
		contentType : "application/json",
		dataType : 'json',
		...
		error : function(request, textStatus, errorThrown) {
			$('p[id=idGlobalSuccessMessage]').html("New column added successfuly");
            $('.alert').hide();
            $('.alert-danger').hide();
            $('div[id=idGlobalSuccess]').show();
            $('#loading').hide();
...
$(document).delegate('#idConfirmDelete', 'click', function(e) {
	...
	$.ajax({
		url : getContextPath() + '/buyer/deletePoNewField',
		data : {
			poId : poId,
			label : label
		},
		...
		error : function(request, textStatus, errorThrown) {
            ...
            $('p[id=idGlobalSuccessMessage]').html("Column deleted successfully");
            $('.alert').hide();
            $('.alert-danger').hide();
            $('div[id=idGlobalSuccess]').show();
            $('#loading').hide();
...
function renderGrid(item) {
	...
	// Define an array of field labels and their corresponding field values
    const fields = [
        { label: item.po?.field1Label, field: item.field1 },
        { label: item.po?.field2Label, field: item.field2 },
        { label: item.po?.field3Label, field: item.field3 },
        { label: item.po?.field4Label, field: item.field4 },
        { label: item.po?.field5Label, field: item.field5 },
        { label: item.po?.field6Label, field: item.field6 },
        { label: item.po?.field7Label, field: item.field7 },
        { label: item.po?.field8Label, field: item.field8 },
        { label: item.po?.field9Label, field: item.field9 },
        { label: item.po?.field10Label, field: item.field10 }
    ];

    // Iterate over the fields array
    fields.forEach((field, index) => {
        if (field.label !== undefined) {
            // Add a table cell for each field
            table += '<td class="width_200 width_200_fix align-center">&nbsp;</td>';
            countNewData++; // Increment the counter

            // Add the header before the third last header in the table
            $('.header_table.header').find('th:nth-last-child(3)').before(
                '<th class="hed_4 width_200 width_200_fix align-center extraFieldHeaders">' +
                field.label +
                '</th>'
            );
        }
	...
	if (item.children != undefined && item.children != null) {
		...
		$.each(item.children, function(i, child) {
			...
			if(item.po.decimal != null && item.po.decimal !==undefined && child.quantity !==undefined){
				table += '<td class="width_100 width_100_fix align-center">' + ReplaceNumberWithCommas(child.quantity.toFixed(item.po.decimal)) + '</td>';
			}else{
				table += '<td class="width_100 width_100_fix align-center">' + ReplaceNumberWithCommas(child.quantity) + '</td>';
			}
			if(item.po.decimal != null && item.po.decimal !==undefined && child.unitPrice!==undefined){
				table += '<td class="width_100 width_100_fix align-center">' + ReplaceNumberWithCommas(child.unitPrice.toFixed(item.po.decimal)) + '</td>';
			}else{
			...
			if(item.po.decimal != null && item.po.decimal !==undefined && child.totalAmount !==undefined){
				table += '<td class="width_120 width_120_fix align-center">' + ReplaceNumberWithCommas(child.totalAmount.toFixed(item.po.decimal)) +'</td>';
			}else{
				table += '<td class="width_120 width_120_fix align-center">' + ReplaceNumberWithCommas(child.totalAmount) +'</td>';
			}
			if(item.po.decimal != null && item.po.decimal !==undefined && child.taxAmount !==undefined){
				table += '<td class="width_120 width_120_fix align-center">' + ReplaceNumberWithCommas(child.taxAmount.toFixed(item.po.decimal)) + '</td>';
			}else{
				table += '<td class="width_120 width_120_fix align-center">' + ReplaceNumberWithCommas(child.taxAmount) + '</td>';
			}
			if(item.po.decimal != null && item.po.decimal !==undefined && child.totalAmountWithTax !==undefined){
				table += '<td class="width_200 width_200_fix align_center totalAmountWithTax">' + ReplaceNumberWithCommas(child.totalAmountWithTax.toFixed(item.po.decimal)) + '</td>';
			}else{
			...
			if ((productItem.length > 2 || productItem.length == 0) && suppId.length == 0) {
				$.ajax({
					...
					success : function(data) {
						...
						if (data != '' && data != null && data.length > 0) {
							$.each(data, function(key, value) {
								html += '<option data-unit="' + value.uom.uom + '" data-tax="' + value.tax + '" data-name="' + value.productName + '" data-price="' + value.unitPrice + '"  value="' + value.id + '">' + value.productCode + ' - '
...
$("#itemName").change(function() {
	...
	$.ajax({
		...
		success : function(data) {
		    console.log("Hasil response ",data.item.id)
		    $('#productItemId').val(data.item.id)
			console.log("poDecimal  >>  " + poDecimal + " data.item.unitPrice " + data.item.unitPrice);
			if(poDecimal != null && poDecimal !==undefined && data.item.unitPrice !== undefined){
				$('#itemUnitPrice').val(ReplaceNumberWithCommas(data.item.unitPrice.toFixed(poDecimal)));
			}else{
