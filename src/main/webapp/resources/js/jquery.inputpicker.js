/**
 * jQuery-inputpicker - A jQuery input picker plugin. It supports showing multiple columns select in input. Copyright (c) 2017 Ukalpa -
 * https://ukalpa.com/inputpicker License: MIT
 */
	...
	if (typeof define === 'function' && define.amd) {
		...
		(function($) {
			...
			methods = {
				init : function(options) {
					return this.each(function() {
						...
						}
						{
							...
							var inputpicker_div = $(
									"<div id=\"inputpicker-div-" + uuid + "\" class=\"inputpicker-div\" data-uuid=\"" + uuid + "\" style=\"position:relative;overflow:auto;height:100%;\">" + "<span class=\"inputpicker-arrow\" data-uuid=\"" + uuid
											+ "\" onclick=\"$(this).parent().find('input').inputpicker('toggle');event.stopPropagation();\" style=\"" + (input_disabled ? "display:none;" : "") + "\"  ><b></b></span>" + "</div>").append(input).on(
									'click', function(e) {
							...
							original.addClass('inputpicker-original').addClass('inputpicker-original-' + uuid).attr('tabindex', -1).data('inputpicker-input', input).data('inputpicker-original-css', {
							...
							input.data('inputpicker-original', original).prop('autocomplete', 'off');
							...
							// input Events
							input.on('focus.inputpicker', _eventFocus).on('blur.inputpicker', _eventBlur).on('keydown.inputpicker', _eventKeyDown).on('keyup.inputpicker', _eventKeyUp);
						...
						var settings = $.extend({
							cache : {}
						}, $.fn.inputpicker.defaults, _options, Array.isArray(options) ? {
							data : options
						} : options);

						// Set default value for fieldText
						if (!settings['fieldText'])
							settings['fieldText'] = settings['fieldValue'];
				...
				/**
				 * Load data manually when having changed url or params It may trigger change if the value was changed
				 * 
				 * @param func -
				 *            callback
				 */
				loadData : function(data, func) {
					return this.each(function() {
						var input = _i($(this));
						_loadData.call(input, input, data, function(input, data) {
				...
				destroy : function(options) {
					return this.each(function() {
						/*
						 * 
						 * original .addClass('inputpicker-original') .addClass('inputpicker-original-' + uuid) .attr('tabindex', -1)
						 * .data('inputpicker-input', input) .data('inputpicker-original-css', { 'top': original.css('top'), 'position':
						 * original.css('position') }) .css({ 'position': 'fixed', 'top': '-1000px' });
						 */
				...
				/*
				 * // Replace as new function to support selecting by specific field element: function (value, field) { return this.each(function () {
				 * var original = _o($(this)); var input = _i(original); if (typeof value == 'undefined'){ value = original.val(); } var fieldValue =
				 * (typeof field === 'undefined') ? _set(input, 'fieldValue') : field; // _set(input, 'data', _formatData(_set(input, 'fieldValue'),
				 * data)); dd('fieldValue:' + fieldValue);
				 * 
				 * var data = _set(input, 'data'); if ( !data.length) return null; // No data var index_i = -1; for(var i = 0; i < data.length; i++){
				 * if ( data[i][ fieldValue ] == value){ dd(["what:", data[i][ fieldValue ], value]); dd(data[i]); return data[i]; } } dd("return
				 * null"); return null;
				 * 
				 * }); },
				 */
				...
				jumpToPage : function(page) {
					...
					return this.each(function() {
						...
						_loadData.call(input, input, null, function(input, data) {
							_dataRender(input);
						});
			...
			function _initMultiple(input) {
				...
				if (!inputpicker_div.find('.inputpicker-multiple').length) {
					...
					inputpicker_div.attr('class', inputpicker_div.attr('class') + ' ' + input.attr('class')).css('border', input.css('border')).addClass('inputpicker-multiple').css('left', '0px')
					// To measure the length of input
					.append("<span class=\"input-span\" style=\"display: none;\" ></span>");

					input.removeClass().width(1).on('keyup.inputpicker.multiple', function(e) {
						var input = $(this);
						var inputpicker_div = _getInputpickerDiv(input);
			 ...
			 * @param input
			 * @param isSelected -
			 *            true: selected false: unselected, undefined: all
			 * @returns {*}
			 * @private
			 */
			...
			/**
			 * Get wrapped list If div wrapped list doest not exist, initiate it If input is not undefined, init it.
			 * 
			 * @returns {*|null}
			 * @private
			 */
			...
			function _selectModeIsRestore(input) {
				return _set(input, 'selectMode') == 'restore';
			}
			function _selectModeIsActive(input) {
				return _set(input, 'selectMode') == 'active';
			}
			function _selectModeIsCreatable(input) {
				return _set(input, 'selectMode') == 'creatable';
			}
			function _selectModeIsEmpty(input) {
				return _set(input, 'selectMode') == 'empty';
			}
			...
			/**
			 * Get / Set settings Do not use it to set data, use _loadData
			 * 
			 * @param input
			 * @param name
			 * @param value
				...
				if (_set(input, 'headShow')) {
					...
					for (var i = 0; i < fields.length; i++) {
						...
						if (i != 4)
							...
							output += '<th style="display:none;">' + text + '</th>';
			...
			function _renderPaginationFooter(input) {
				...
				if (fields.length > 0 && _pagination(input)) {
					...
					output += "<div style=\"float:right;padding-right:5px;\">" + "<a href=\"javascript:void(0);\" onclick=\"$('#inputpicker-" + _uuid(input) + "').inputpicker('jumpToPage', '1');\">First</a>"
							+ "<a href=\"javascript:void(0);\" onclick=\"$('#inputpicker-" + _uuid(input) + "').inputpicker('jumpToPage', '" + prev_page + "');\">Prev</a>" + "<span class=\"current-page\">" + current_page + "</span>"
							+ "<a href=\"javascript:void(0);\" onclick=\"$('#inputpicker-" + _uuid(input) + "').inputpicker('jumpToPage', '" + next_page + "');\">Next</a>" + "<a href=\"javascript:void(0);\" onclick=\"$('#inputpicker-" + _uuid(input)
							+ "').inputpicker('jumpToPage', '" + last_page + "');\">Last</a>" + "</div>";
			...
			function _matchActiveInRender(input) {
				...
				for (var x = 0; x < elements.length; x++) {
					...
					if (tr_new_active) { // Should set active
						...
						// dd(tr_active, tr_active.position().top, tr_active.outerHeight(), wrapped_list.scrollTop(), wrapped_list.scrollTop() -
						// tr_active.outerHeight() );
			...
			/**
			 * Load data if necessary If url, load from url Else if data is not null, load data into settings Else read data from setting
			 * 
			 * @param input
			 * @private
			 */
			function _loadData(input, data, func) {
				...
				if (_isMSIE())
					input.addClass('loading-msie-patch');
				...
				if (_set(input, 'url')) {
					...
					_execJSON(input, param, function(ret) {
						...
						if (_isMSIE())
							input.removeClass('loading-msie-patch');
			...
			function _setValueForMultiple(input, value) {
				...
				var o_v = old_original_value, n_v = new_values.sort().join(delimiter);
			 ...
			 * 
			 * @param isActive
			 *            undefined all elements true active false unactive
			 * @private
			 */
			...
			function _generateUid() {
				if (!window.inputpickerUUID)
					window.inputpickerUUID = 0;
			...
			function _eventKeyUp(e) {
				var input = $(this);
				// hard code value for enable filed when we make empty data for item
			...
			$.fn.inputpicker.defaults = {
				...
				/**
				 * The action after pressing 'tab' restore: Use the previous value, the change event is not raised. active: Use the active option new:
				 * Use the current keyword, null : Set the word is null
				 */
				selectMode : 'restore',

				/**
				 * True - show head False
				 */
				...
				/**
				 * Fields Store fields need to been shown in the list (Sting) - 'value' (Object) - {name:'value', text:'Value'}
				 */
				fields : [],
				...
				/**
				 * The field shown in the input Will use fieldValue if empty
				 */
				fieldText : '',
				...
				/**
				 * True - filter rows when changing the input content False - do not do any spliation
				 */
				filterOpen : false,

				/**
				 * Choose the method of filtering 'start' - start filtering from the beginning others - all content matches
				 */
				filterType : '', // 'start' - start from beginning or ''

				/**
				 * Choose the fields need to be filtered (String)'name' - one field (Array)['name', 'value'] - multiple fields
				 */
				filterField : '',
