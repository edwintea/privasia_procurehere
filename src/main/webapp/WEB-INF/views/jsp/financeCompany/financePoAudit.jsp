...
<div class="panel sum-accord marg-top-5">
	<div class="panel-heading">
		<h4 class="panel-title">
			<a data-toggle="collapse" class="accordion" href="#collapseAudit"> History </a>
		</h4>
	</div>
				...
				<table class="tabaccor padding-none-td table" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-bottom:0px;">
					<thead>
						<tr>
							...
							<th class="align-left width_200_fix">Referral Fee</th>
							<th class="align-left width_200">Remarks</th>
						</tr>
					</thead>
					<tbody>
						<c:forEach items="${financeprAuditList}" var="audit">
							<tr>
								...
								<td class="align-left width_200_fix">${audit.referralFee}</td>
								<td class="align-left width_200_fix">${audit.remark}</td>
								<%-- <td class="align-left width_200 white-space-pre" style="color: ${audit.action == 'CANCELLED' ? '#e74c3c' : (audit.action == 'TRANSFERRED' ? '#29b765' :  (audit.action == 'DELIVERED' ? '#0095d5' : ''))};">${audit.description}</td> --%>
							</tr>
						</c:forEach>
					</tbody>
				</table>
