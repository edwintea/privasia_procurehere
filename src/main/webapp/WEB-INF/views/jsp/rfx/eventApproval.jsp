...
<div class="dynamic-approval col-md-10">
		...
		<div class="form_field" id="idReminderSettings" style="margin-left: -20px;">
			<div class="row">
				<div class="col-sm-11 col-md-11 col-xs-11 ${event.status == 'SUSPENDED' ? 'disabled' : '' }" >
					<form:checkbox path="enableApprovalReminder" id="enableApprovalReminder" class="custom-checkbox" />
			...
			<div class="form_field" id="approvalReminderHourDiv" style="padding-top: 8px; padding-left: 7px; ">
			  <div class="form_field" style="margin-bottom: 0px !important; ">
				<label class="col-sm-3 control-label wd-cl" style="margin-top: 8px;" ><spring:message code="approval.reminder.hour" /></label>
				 <div class="col-sm-3 pd-wd ${event.status == 'SUSPENDED' ? 'disabled' : '' }">
			  ...
			  <div class="form_field" style="margin-bottom: 0px !important; ">	
				...
				<label class="col-sm-remi control-label" style="margin-top: 16px;"><spring:message code="reminder.emails" /></label>
				 ...
				 <div class="${event.status == 'SUSPENDED' ? 'disabled' : '' }" style="margin-top: 16px;">
				 <form:checkbox path="notifyEventOwner" id="idNotifyEventOwner" class="custom-checkbox" />
			     <label class="notify-change col-sm-remi control-label"><spring:message code="event.notify.event.owner" /></label>
	   	         </div>
	...
	<div class="row cloneready">
		<div class="col-md-3 col-sm-4">
			<button type="button" class="btn btn-plus btn-info hvr-pop " id="addSelect" ${event.status =='SUSPENDED'  || (!empty rfxTemplate ? (tf:rfxApprovalReadonly(rfxTemplate) ? 'disabled="disabled"' : '') : '') ? 'disabled' : ''} data-placement="top"><spring:message code="application.addapproval.button" /></button>
		</div>
	</div>
	...
	<div class="approVAlRouteBox " style="max-height: unset !important; overflow-y: unset !important; overflow-x: unset !important;">
		<c:forEach items="${event.approvals}" var="approval" varStatus="status">
			<div id="new-approval-${status.index + 1}" class="row new-approval">
				<c:if test="${approval.active}">
					<c:forEach items="${event.approvals[status.index].approvalUsers}" var="usr">
				...
				<div class="col-md-7 col-sm-7 col-xs-8 approval-chosen-multiselect ${event.status =='SUSPENDED' || approval.done || (!empty rfxTemplate ? (tf:rfxApprovalReadonly(rfxTemplate)) : false) ? 'disabled':''}">
					<span class="dropUp">
					<form:select data-validation="required" id="multipleSelectExample-${status.index}" path="approvals[${status.index}].approvalUsers" cssClass="tagTypeMultiSelect user-list-all chosen-select  ${event.status =='SUSPENDED' || approval.done || (!empty rfxTemplate ? (tf:rfxApprovalReadonly(rfxTemplate)) : false) ? 'disabled':''}" data-placeholder="${approversplace}" multiple="multiple">
				...
				<div class="col-sm-6 col-md-3 col-xs-4">
					<div class="btn-address-field pt-0">
						<button type="button" class="btn marg-mins ${approval.approvalType == 'OR' ? 'btn-success' : 'btn-warning'} dropdown-toggle ${ (event.status =='SUSPENDED' || approval.active || approval.done || (!empty rfxTemplate ? (tf:rfxApprovalReadonly(rfxTemplate)) : false))  ? 'disabled':''}" data-toggle="dropdown">
						...
						<button class="btn btn-plus btn-info hvr-pop hvr-rectangle-out removeApproval  ${event.status =='SUSPENDED' || approval.active || approval.done  || (!empty rfxTemplate ? (tf:rfxApprovalReadonly(rfxTemplate)) : false ) ? 'disabled':''}">
...
function autoSave(){
	...
	var index;
	index = ${fn:length(event.approvals) + 1};
	...
	//	document.getElementById("addSelect").addEventListener("click", function(e) {
		$("#addSelect").click(function(e) {
			...
			//$("#multipleSelectExample-'+(index - 1) > .chosen-search").append('<i class="glyph-icon icon-search"></i>');
			//$("#multipleSelectExample-'+(index - 1) > .chosen-single div").html('<i class="glyph-icon icon-caret-down"></i>');
		...
		if(document.getElementById('enableApprovalReminder') != null){
			if(document.getElementById('enableApprovalReminder').checked) {
				$('#approvalReminderHourDiv').show();
			}else{
				$('#approvalReminderHourDiv').hide();
			} 
		}
		$("#enableApprovalReminder").change(function() {
		    if(this.checked) {
		    	$('#approvalReminderHourDiv').show();
