...
<div id="page-content-wrapper">
    <div id="page-content">
        <div class="container">
            ...
            <input type="hidden" class="addBillOfQuantityControl" value="${event.addScheduleOfRate}">
                ...
                <div class="white-bg border-all-side float-left width-100 pad_all_15 marg-bottom-10">
                    <div class="margin-bottom-10">
                        <div class="meeting2-heading">
                            ...
                            <button class="pull-right btn ph_btn_midium btn-info toggleCreateBq hvr-pop hvr-rectangle-out editBqItem" type="button" data-toggle="tooltip" data-id="${eventBq.id}" data-original-title='<spring:message code="tooltip.edit.sor" />' style="margin-right: 20px; margin-top: 7px; height: 30px; line-height: 1;"><spring:message code="tooltip.edit.sor" /></button>
                        </div>
                        <div class="import-supplier-inner-first-new pad_all_15 global-list form-middle" id="bqDisplay" style="display: none;">
                            <form id="bqFormAddEdit" class="form-horizontal bordered-row has-validation-callback" action="" method="post">
                                <input type="hidden" name="eventId" id="eventId" value="${event.id}"> <input type="hidden" name="eventType" id="eventType" value="${eventType}"> <input id="bqId" name="id" value="${eventBq.id}" type="hidden">
                                <input id="editBqName" name="editBqName" value="<c:out value='${eventBq.name}'/>" type="hidden">
                                <input id="editBqDesc" name="editBqDesc" value="<c:out value='${eventBq.description}'/>" type="hidden">
                                <input id="eventDecimal" value="${event.decimal}" type="hidden">
                                <header class="form_header"></header>
                                <jsp:include page="createAddSor.jsp" />
                ...
                <div class="column_button_bar">
                    ...
                    <div class="pull-right ${(event.status == 'SUSPENDED' and (event.suspensionType == 'KEEP_NOTIFY' or event.suspensionType == 'KEEP_NO_NOTIFY'))?'disabled':''}">
                        <div class="row" style="margin-right: 0px;">
                            <input name="bqItemSearch" type="search" id="bqItemSearch" placeholder='<spring:message code="sor.search.itemname.placeholder" />' class="form-control" />
                        </div>
                    </div>
                ...
                <form id="addEditSectionForm">
                    ...
                    <div class="col-md-12 col-sm-12">
                        <div class="s1__frmbtn_block">
                            ...
                            <button class="btn ph_btn_midium btn-black hvr-pop hvr-rectangle-out1 bqCancelSec" onclick="resetForm();"><spring:message code="application.cancel" /></button>
                        </div>
                    </div>
                </form>
            </div>
...
<%--                    <spring:message code="event.document.unitpricing" var="unitpricing" />--%>
                    ...
                    <div class="col-md-12 col-sm-12 col-xs-12">
                        <div class="s1__frmbtn_block">
                            ...
                            <button type="button" class="btn ph_btn_midium btn-black hvr-pop hvr-rectangle-out1 bqCancelSec" onclick="resetForm();"><spring:message code="application.cancel" /></button>
                        </div>
                    </div>
                </form>
            </div>
                            ...
                            <tr class="addColumsBlock" data-pos="1">
                                <td class="width_300">
                                    <div class="form-group s1-mrg-10">
                                        <input type="text" class="form-control" id="field1Label" name="field1" data-validation="required length" data-validation-length="max32" placeholder='<spring:message code="prtemplate.name.placeholder" />' value="<c:out value='${eventBq.field1Label}' />">
                                    </div>
                                </td>
                                <td class="width_300">
                                    <div class="form-group s1-mrg-10">
            ...
            <div class="table_fix_header">
                <table class="header_table header bq_font" width="100%" cellpadding="0" cellspacing="0" id="table_id">
                    <tr>
                        ...
                        <th class="hed_5 width_100 width_115_fix align_center ">
                        <th class="hed_5 width_100 width_115_fix align_center ">
                        <c:if test="${not empty eventBq.field1Label}">
                                            ...
                                            <div class="menuDiv sub-color-change">
                                                <table style=" background: #eef7fc;" class="table data ph_table diagnosis_list sorted_table" id="table_id">
                                                    <tr class="sub_item item_${bqchild.level}_${bqchild.order}"  data-id="${bqchild.id}" data-parent="${bq.id}">
                                                        ...
                                                        <td class="width_200 width_200_fix align-left"><span class="item_name itemNameD">${bqchild.itemName}
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="table_f_action_btn">
                    <form class="bordered-row" id="submitPriviousForm" method="post" style="float: left;" action="${pageContext.request.contextPath}/buyer/${eventType}/sorListPrevious">
                    ...
                    <form class="bordered-row" id="submitNextForm" method="post" style="float: left;" action="${pageContext.request.contextPath}/buyer/${eventType}/sorNext">
                        <input type="hidden" id="eventId" value="${eventId}" name="eventId"> <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}" />
...
<script type="text/javascript">
    function resetForm() {
        ...
        document.getElementById('addNewItems').reset();
        ...
        // Remove any displayed validation messages (if you are using a validation library)
        document.querySelectorAll('.error-message').forEach(function(error) {
            error.remove();
        });

        // Reset select elements (if using a plugin like Chosen or Select2)
...
<script type="text/javascript" src="<c:url value="/resources/js/view/eventSor.js?2"/>"></script>
<script>
    ...
    <c:if test="${!event.addScheduleOfRate}">
    $(window).bind('load', function() {
