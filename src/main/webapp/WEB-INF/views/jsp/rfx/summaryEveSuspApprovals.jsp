...
<div class="panel sum-accord Approval-tabs">
	<div class="panel-heading">
		<h4 class="panel-title">
			...
			<c:if test="${eventPermissions.owner and event.status == 'PENDING'}">
				<button class="editSuspApprvPopupButton editSuspBtn" data-toggle="dropdown"  title="Edit" style="display: none;">
	...
	<div id="collapseSuspAppr" class="panel-collapse collapse accortab in">
		<div class="panel-body eleven-contant pad_all_15">
			...
			<c:forEach items="${event.suspensionApprovals}" var="approval">
	...
	<div class="remark-tab event_info pad0">
		<h4><spring:message code="summarydetails.approval.suspension.comments" /></h4>
		<div class="panel-body pad_all_15">
			<c:forEach items="${suspComments}" var="comment">
		...
		<c:if test="${event.status == 'SUSPENDED' and showApprovalRemark and eventPermissions.activeSuspensionApproval}">
			<form id="suspApprovedRejectForm" method="post" action="">
				<div class="row">
				...
				<div class="col-md-12 col-xs-12 col-ms-12 marg-top-20 marg-bottom-20">
					<button type="button" id="suspApprovedButton" class="btn btn-info ph_btn hvr-pop hvr-rectangle-out" style="margin-right: 20px;"><spring:message code="buyer.dashboard.approve" /></button>
					<button type="button" id="suspRejectedButton" class="btn btn-black ph_btn hvr-pop hvr-rectangle-out1"><spring:message code="application.reject" /></button>
				</div>
			</form>
		</c:if>
	</div>
