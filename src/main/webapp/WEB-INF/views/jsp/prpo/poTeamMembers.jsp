				...
				<div class="ia-invite-controls-area">
					<div class="group">
						<div class="input-group mrg15T mrg15B ${buyerReadOnlyAdmin ? 'disabled' : ''}">
							<select id="TeamMemberList" class="user-list-all chosen-select" selected-id="approver-id" name="approverList1" cssClass="form-control chosen-select">
								<option value=""><spring:message code="prsummary.select.team.member" /></option>
                                <c:forEach items="${userList}" var="users1">
                                    <c:if test="${users1.id == '-1' }">
                                        template += '<option value="-1" disabled >${users1.user.name}</option>';
                                    </c:if>
                                    <c:if test="${users1.id != '-1' }">
                                        template += '<option value="${users1.id}" >${users1.user.name}</option>';
                                    </c:if>
                                </c:forEach>
							</select>
