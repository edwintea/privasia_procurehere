		...
		<c:forEach items="${po.approvals}" var="approval" varStatus="status">
			<div id="new-approval-${status.index + 1}" class="row new-approval">
				...
				<div class="col-md-7 col-sm-7 col-xs-8 approval-chosen-multiselect ${buyerReadOnlyAdmin ? 'disabled' : ''} ${approval.done || (!empty prTemplate ? (tf:prApprovalReadonly(prTemplate)) : false) ? 'disabled':''}">
					...
					<span class="dropUp">
					<form:select data-validation="required" id="multipleSelectExample-${status.index}" path="approvals[${status.index}].approvalUsers" cssClass="tagTypeMultiSelect user-list-all-approval chosen-select  ${approval.done || (!empty prTemplate ? (tf:prApprovalReadonly(prTemplate)) : false) ? 'disabled':''}"
...
<!-- set parameter here-->
<c:set var="minApprovalCount" value="${prTemplate.minimumPoApprovalCount}" />
...
<c:set var="isApprovalRequired" value="${tf:poApprovalRequired(prTemplate)}" />
...
<script type="text/JavaScript">
	...
	//ph-4113
	index = ${fn:length(po.approvals) + 1};
	var minApprovalCount='${minApprovalCount}';
	var isApprovalReadOnly='${isApprovalReadOnly}';
	var isApprovalVisible='${isApprovalVisible}';
	var prTemplateId='${prTemplateId}';
	let userId = '${userId}'

	let currentPoApprovalLevel=parseInt($('.new-approval').length);
	minApprovalCount = parseInt(minApprovalCount);
	let passedApproval = minApprovalCount === 0 || currentPoApprovalLevel >= minApprovalCount;

	var  checkCurrentApprovalCount=()=>{
		let ret = {'minApprovalCount':minApprovalCount,'currentPoApprovalLevel':currentPoApprovalLevel,passed:false}

		if(parseInt(currentPoApprovalLevel) >= parseInt(minApprovalCount)){
			ret.passed=true;
			$('.ajax-msg').hide();
		}
	...
	$(document).ready(function() {
            ...
            document.getElementById("addSelect").addEventListener("click", function(e) {
                ...
                template += '<span class="dropUp"><select data-validation="required" id="multipleSelectExample-'+(index - 1)+'" name="approvals[' + (index - 1) + '].approvalUsers" class="user-list-all-approval chosen-select tagTypeMultiSelect newApprovalUser" data-placeholder=<spring:message code="createrfi.approvalroute.approvers.placeholder"/>  multiple>';
                ...
                updateUserForPoApprovalList('',$('#multipleSelectExample-'+(index - 1)),'ALL_USER');
                $(".approVAlRouteBox").animate({ scrollTop: $(".approVAlRouteBox")[0].scrollHeight}, 1000);
                index++;

                $('.newApprovalUser').change(function(){
                    currentPoApprovalLevel++;
                    checkCurrentApprovalCount();
                });
            });

            $(document).delegate('.removeApproval', 'click', function(i,e) {
                $(this).closest(".new-approval").remove();
                $(this).closest(".approval_id_hidden").remove();

                if(currentPoApprovalLevel!=0){
                    currentPoApprovalLevel--;
                }
