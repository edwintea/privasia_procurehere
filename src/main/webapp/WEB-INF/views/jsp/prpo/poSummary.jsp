...
<div class="white-bg border-all-side float-left width-100 pad_all_15">
	<div class="row">
        ...
        <c:if test="${ po.status eq 'ORDERED' or po.status eq 'SUSPENDED' or po.status eq 'REVISE' or po.status eq 'PENDING' or (po.status eq 'READY' and (isLoggedInUserApprover or bussUnitUser))}">
		...
		<c:if test="${(po.status ne 'REVISE' and po.status ne 'SUSPENDED' and po.status ne 'ORDERED' and po.status ne 'PENDING' and (isCreatorAndMember))}">
			...
			<c:if test="${po.fromIntegration}">
				<div class="col-sm-5 col-md-5 col-xs-6">
					<c:if test="${not empty po.supplier}">
						<div class="">
							<!--
							<h5>${po.supplier.supplier.companyName}</h5>
							-->
        ...
        <div class="upload_download_wrapper clearfix marg-top-10 event_info Approval-tab">
            <h4>
                ...
                <c:if test="${!prTemplate.approvalPoReadOnly}">
                    <c:choose>
                        <c:when test="${po.status eq 'SUSPENDED' or po.status eq 'DRAFT' or po.status eq 'PENDING'}">
                            <c:if test="${eventPermissions.owner}">
