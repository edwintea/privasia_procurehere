		...
		<div class="content-box row">
			...
			<div class="mail-toolbar clearfix pad_all_10">
				<div class="float-left dd_left">
					...
					<c:choose>
						<c:when test="${isCreatorAndMember}">
						...
						<c:otherwise>
							<a class="btn btn-default" id="compose_new" title="" href="javascript:void(0);" disabled>
								<img src="<c:url value="/resources/assets/images/mail_edit.png"/>" alt="" data-toggle="tooltip" data-placement="top" title="Compose New Mail">
							</a>
						</c:otherwise>
					</c:choose>
		...
		<div class="content-box">
			<form class="form-horizontal width-form-set" id="login-validation">
				...
				<div class="form-group row">
					...
					<div class="col-sm-8 disabled" id="selectSuppliers">
						<input data-validation="required length" style="margin-top:2%;" id="toMsg" tabindex="-1" maxlength="250" data-validation-length="max250" data-supplier="${po.supplier.supplier.id}" value="${po.supplierName}" type="text"  class="form-control">
					</div>
				</div>
				<div class="form-group row">
	...
	<div class="col-md-12">
		<div class="content-box">
			...
			<div class="pad_all_10">
				...
				<div id="attchMentDatas" style="display:none;"></div>
...
$(function(){
    ...
    $(document).delegate('#replaymail', 'click', function() {
        ...
        var supplier = $('#toMsg').data('supplier');
        var supplierName = $('#toMsg').val();

        var recHtml="<input data-validation='required length' data-supplier='${po.supplier.supplier.id}' value='${po.supplierName}' type='text'  class='form-control' disabled>";
    ...
    $(document).delegate(".viewDetailMsg", 'click', function(e) {
        ...
        $('.attachmentOnNoReply').html($('#attchMentDatas').html()).addClass('hidden');
    });
    ...
    $("#sendReply").click(function(e) {
        ...
        // Send AJAX request
        $.ajax({
            url: getContextPath() + '/buyer/' + getEventType() + '/sendMessage/${eventId}',
            data: form_data,
            cache: false,
            xhr: function() {
    ...
    $("#nextMail, #prevMail").click(function() {
        var ajaxUrl = getContextPath() + '/buyer/' + getEventType() + '/eventMessages/${eventId}';
    ...
    function renderMailList(data) {
        ...
        $(data).each(function(i, message) {
            ...
            if (message.fileName != '' && message.fileName != undefined) {
                ...
                html += '<div class="image_sec marg-top-10 border-none"><a class="bluelink" href="' + getContextPath() + '/buyer/' + getEventType() + '/downloadMessageAttachment/' + message.id + '"><img src="' + getContextPath() + '/resources/images/attach_file.png"/> Attachment : <span>' + message.fileName + '</span></a></div>';
                html += '</div>';
            }
            html += '</td>';
            ...
            html += "<div  class='hmsgBody hide'>" + messagelocal + "</div>";
            ...
            if (!(typeof message.replies === 'undefined') && message.replies.length != 0) {
                ...
                $(message.replies).each(function(i, reply) {
                    ...
                    if (reply.fileName != '' && reply.fileName != undefined) {
                        html += '<img src="' + getContextPath() + '/resources/images/attach_file.png"/><div class="attchMentValus hide">';
                        html += '<div class="image_sec marg-top-10 border-none"><a class="bluelink" href="' + getContextPath() + '/buyer/' + getEventType() + '/downloadMessageAttachment/' + reply.id + '"><img src="' + getContextPath() + '/resources/images/attach_file.png"/> Attachment : <span>' + reply.fileName + '</span></a></div>';
                        html += '</div>';
                    }
                    html += '</td>';
