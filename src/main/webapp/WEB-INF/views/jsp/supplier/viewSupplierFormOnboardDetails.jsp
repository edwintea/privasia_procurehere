...
<link rel="stylesheet" type="text/css" href="<c:url value="/resources/assets/elements/procurehere1.css"/>">
			...
			<div class="Section-title title_border gray-bg mar-b20">
				...
				<c:if test="${supplierForm.status eq 'PENDING'}">
				<spring:message code="supplier.form.status" /> :  DRAFT
				</c:if>
				<c:if test="${supplierForm.status ne 'PENDING'}">
			 ...
			 <div class="clear"></div>
				...
				<div class="tab-main-inner pad_all_15">
					...
					<div class="cqlistDetails">
								<form:form id="supplierForm" action="${pageContext.request.contextPath}/supplier/submitSupplierViewOnboardform/${supplierForm.id}?${_csrf.parameterName}=${_csrf.token}" method="post" modelAttribute="supplierFormSubmissionItemPojo" enctype="multipart/form-data">
									...
									<div class="Gen-ques">
										...
										<div class="Gen-ques-inner">
											...
											<input type="hidden" id="buyerId" value="${supplierForm.buyer.id}" />
																		...
																		<div class="row" id="" style="margin-top:10px;">
																			<ul>
																				...
																				<li><c:url var="download" value="/supplier/downloadSupplierDocument/${eventDocument.id}" />
	                                                                             <a href="${download}" data-placement="top" class="buyerItemAttachment" title="Download"> ${eventDocument.fileName}</a>
																				</li>
																				</c:forEach>
																			</ul>
																		</div>
...
$('#draftbutton').click(function() {
	$(this).addClass('disabled');
	$('#supplierForm').attr('action', getContextPath() + "/supplier/saveViewOnbaordFormAsDraft/${supplierForm.id}?${_csrf.parameterName}=${_csrf.token}");
	$('#supplierForm').submit();
});
...
$(window).bind('load', function() {
	var allowedFields = '#cancelRequest, #sentRequest, .accordion, .collapsed, #remarksId, #idWebsite, #token-input-demo-input-local, #closeButtonId, .formAttachmentDownload, .buyerItemAttachment';
	...
	$.ajax({
		...
		success : function(data, textStatus, request) {
			if (data == true) {
				...
				window.location.href = getContextPath() + "/supplier/viewRequestedAssociateBuyer/" + buyerId + "?success=" + $.trim(message);
			} else {
