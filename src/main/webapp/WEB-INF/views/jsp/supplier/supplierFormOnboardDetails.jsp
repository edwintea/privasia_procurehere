			 ...
			 <div class="clear"></div>
				...
				<div class="tab-main-inner pad_all_15">
					...
					<div class="cqlistDetails">
								<form:form id="supplierForm" action="${pageContext.request.contextPath}/supplier/submitSupplierOnboardform/${supplierForm.id}?${_csrf.parameterName}=${_csrf.token}" method="post" modelAttribute="supplierFormSubmissionItemPojo" enctype="multipart/form-data">
...
$('#draftbutton').click(function() {
	$(this).addClass('disabled');
	$('#supplierForm').attr('action', getContextPath() + "/supplier/saveOnbaordFormAsDraft/${supplierForm.id}?${_csrf.parameterName}=${_csrf.token}");
	$('#supplierForm').submit();
});
	...
	$.ajax({
		...
		success : function(data, textStatus, request) {
			if (data == true) {
				...
				window.location.href = getContextPath() + "/supplier/requestToAssociateBuyer/" + buyerId + "?success=" + $.trim(message);
			} else {
