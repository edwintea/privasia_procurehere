...
<div id="page-content" view-name="footerList">
	<div class="container col-md-12">
		...
		<ol class="breadcrumb">
			...
			<li class="active">
				<spring:message code="supplier.footer.list" />
			</li>
		</ol>
						...
						<div class="row">
							<div class="col-xs-12">
								<div class="ph_tabel_wrapper scrolableTable_UserList">
									<table id="tableList" class=" display table table-bordered noarrow" cellspacing="0" width="100%">
										<thead>
											<tr class="tableHeaderWithSearch">
												...
												<th search-type="text"><spring:message code="footer.title" /></th>
												<th search-type="select"  search-options="footerTypeList"><spring:message code="footer.type" /></th>
									...
									<spring:url value="/supplier/footer" var="createUrl" htmlEscape="true" />
									<a href="${createUrl}" class="btn btn-info top-marginAdminList"> <spring:message code="footer.create" /></a>
								</div>
	...
	<div class="modal-dialog for-delete-all reminder">
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<label> <spring:message code="footer.delete" />
				</label>
			</div>
			<div class="modal-footer pad_all_15 float-left width-100 border-top-width-1">
				<a id="idConfirmDelete" class="btn btn-info ph_btn_small pull-left hvr-pop hvr-rectangle-out" href="${pageContext.request.contextPath}/supplier/deleteFooter?id=" title="Delete"> <spring:message code="application.delete" />
				</a>
...
$('document').ready(function(){
	var table=$('#tableList').DataTable({
		...
		"searching" : true,
		"ajax":{
			"url":getContextPath()+"/supplier/footerListData",
			"data":function(d){
			},
		...
		"columns":[{
				...
				"mRender":function(data,type,row){
					...
					ret += '<a href="editFooter?id=' + row.id +'" title="Edit"><img src="${pageContext.request.contextPath}/resources/images/edit1.png"></a>'; 
	...
	$('#tableList thead tr:nth-child(1) th').each(function(i) {
		...
		if (!(title == "Actions") && $(this).attr('search-type') != '') {
			if ($(this).attr('search-type') == 'select') {
				...
				if (optionsType == 'footerTypeList') {
					<c:forEach items="${footerTypeList}" var="item">
					htmlSearch += '<option value="${item}">${item}</option>';
					</c:forEach>
				}
...
function doCancel() {
	var link = $("a#idConfirmDelete");
	link.data('href', link.attr("href"));
	link.attr("href", '${pageContext.request.contextPath}/supplier/deleteFooter?id=');
}
</script>