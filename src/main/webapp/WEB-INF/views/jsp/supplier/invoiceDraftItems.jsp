			...
			<table class=" ph_table border-none header parentBlocks" border="0" cellspacing="0" cellpadding="0" width="100%">
				<thead>
					<tr>
						...
						<th class=" align-right width_150 width_150_fix"><spring:message code="label.rftbq.th.unitprice" /> (${invoice.currency})</th>
						...
						<th class="width_250 width_250_fix align-right"><spring:message code="prtemplate.total.amount.tax" /> (${invoice.currency})</th>
					</tr>
				</thead>
			</table>
					...
					<c:forEach items="${itemlist}" var="item">
						<tr id="${item.id}">
							...
							<td class="align-right width_100 width_100_fix">
								<c:if test="${item.order > 0}">
								<fmt:formatNumber var="quantity" type="number" minFractionDigits="${invoice.decimal}" maxFractionDigits="${invoice.decimal}" value="${item.quantity}" />
								<input type="text" class="validate form-control itemValue text-right" data-pos="1"  name="quantity" data-validation="custom number required length" data-validation-regexp="^\d{1,16}(\.\d{1,${invoice.decimal}})?$" data-validation-ignore=",." value="${quantity}"   data-validation-length="1-22"  />
								</c:if>	

							</td>
							<td class=" align-right width_150 width_150_fix">
								<c:if test="${item.order > 0}">
								<fmt:formatNumber var="unitPrice" type="number" minFractionDigits="${invoice.decimal}" maxFractionDigits="${invoice.decimal}" value="${item.unitPrice}" />
								<input type="text" data-pos="2"  name="unitPrice" class="validate form-control itemValue text-right" value="${unitPrice}"  data-validation="custom number required length" data-validation-regexp="^\d{1,16}(\.\d{1,${invoice.decimal}})?$" data-validation-ignore=",."  data-validation-length="1-22"  />
								</c:if>	
							</td>
							<td class=" align-right width_100 width_100_fix">
								<c:if test="${item.order > 0}">
									<fmt:formatNumber var="itemTax" type="number" minFractionDigits="${invoice.decimal}" maxFractionDigits="${invoice.decimal}" value="${item.itemTax}" />
									<input type="text" data-pos="3"  name="itemTax" class="validate form-control itemValue text-right" value="${itemTax}"  data-validation="custom number length" data-validation-optional="true" data-validation-regexp="^\d{1,16}(\.\d{1,${invoice.decimal}})?$" data-validation-ignore=",."  data-validation-length="1-22"  />
								</c:if>
							</td>
							...
							<td class="width_150 width_150_fix align-right"><c:if test="${item.order != '0' }">
									<span class="rowTotalAmount"> <fmt:formatNumber type="number" minFractionDigits="${invoice.decimal}" maxFractionDigits="${invoice.decimal}" value="${item.totalAmount}" /> </span>
								</c:if></td>
							<td class="width_150 align-right width_150_fix"><c:if test="${item.order != '0' }">
									<span class="rowTaxAmount"> <fmt:formatNumber type="number" minFractionDigits="${invoice.decimal}" maxFractionDigits="${invoice.decimal}" value="${item.taxAmount}" /> </span>
								</c:if></td>
							<td class="width_250 width_250_fix align-right"><c:if test="${item.order != '0' }">
									<span class="rowTotalAfterTaxAmount"> <fmt:formatNumber type="number" minFractionDigits="${invoice.decimal}" maxFractionDigits="${invoice.decimal}" value="${item.totalAmountWithTax}" /> </span>
								</c:if>
							</td>
						</tr>
					</c:forEach>
		...
		<div class="total_all total-with-tax-final table-resposive" style="font-size: 16px;">
			<table cellspacing="3" cellpadding="3" style="width: 100%; border-collapse: separate; border-spacing: 8px; margin-right: 20px;">
				<tr>
					<td>&nbsp;</td>
					<td>&nbsp;</td>
					<td class="align-left" style="white-space: nowrap;"><spring:message code="prsummary.total2" /> (${invoice.currency}) :</td>
					<td class="width_250 width_250_fix align-right" style="white-space: nowrap;"><span id="idTotal"><fmt:formatNumber type="number" minFractionDigits="${invoice.decimal}" maxFractionDigits="${invoice.decimal}" value="${invoice.total}" /></span></td>
				</tr>
				<tr>
					...
					<td style="white-space: nowrap; vertical-align: top;" class="width_250 width_250_fix align-right">
						<fmt:formatNumber var="additionalTax" type="number" minFractionDigits="${invoice.decimal}" maxFractionDigits="${invoice.decimal}" value="${invoice.additionalTax}" />
				...
				<tr>
					...
					<td style="white-space: nowrap;" class="align-left">
						<strong><spring:message code="submission.report.grandtotal" /> (${invoice.currency}):</strong>
					</td>
					<td style="white-space: nowrap;" class="width_250 width_250_fix align-right"><strong><span id="idGrandTotal"><fmt:formatNumber type="number" minFractionDigits="${invoice.decimal}" maxFractionDigits="${invoice.decimal}" value="${invoice.grandTotal}" /></span></strong></td>
				</tr>
			</table>
		</div>
	</div>
...
<script type="text/javascript">
	var decimalLimit = ${invoice.decimal};
</script>	
<script type="text/javascript" src="<c:url value="/resources/js/view/dobq.js"/>"></script>
