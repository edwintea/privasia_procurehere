...
<%-- 							  <c:url value="/supplier/ExportSupplierDoReport" var="downloadSupplierDoReport" /> --%>
<%-- 							    <form:form action="${downloadSupplierDoReport}" method="post" id="exportSupplierDoForm" ModelAttribute="doSupplierPojo"> --%>
							  <c:url value="/supplier/ExportSupplierDoCsv" var="downloadSupplierDoCsv" />
							    <form:form action="${downloadSupplierDoCsv}" method="post" id="exportSupplierDoForm" ModelAttribute="doSupplierPojo">  
								<div class="col-md-12 marg-top-10">
									<label><spring:message code="do.filter.By.created.Date" /></label>
									<div class="row">
...
<%-- 											<button id="exportSupplierDo" class="btn btn-sm btn-success hvr-pop marg-left-10 f-r h-37" data-toggle="tooltip" data-placement="top" data-original-title='<spring:message code="do.export.button"/>'> --%>
...
<!-- 											</button> -->
											<button id="exportSupplierDoCsv" class="btn btn-sm btn-success hvr-pop marg-left-10 f-r h-37" data-toggle="tooltip" data-placement="top" data-original-title='<spring:message code="do.export.button"/>'>
												<span class="glyph-icon icon-separator"> <i class="glyph-icon icon-download"></i>
												</span> <span class="button-content"><spring:message code="do.export.button" /></span>
											</button>
										</div>
												...
												<table id="tableList" class="data  display table table-bordered noarrow" cellspacing="0" width="100%">
													<thead>
														<tr class="tableHeaderWithSearch">
															...
															<th search-type="text" class="width_200 width_200_fix align-left"><spring:message code="supplier.doListing.createdby"/></th>
															<th search-type="" class="align-left width_200 width_200_fix"><spring:message code="supplier.doListing.createdDate"/></th>
	...
	<script type="text/javascript">
		$('document').ready(function() {
			...
			var table = $('#tableList').DataTable({
				...
				"searching" : true,
				"ajax" : {
					"url" : getContextPath() + "/supplier/doListData",
					"data" : function(d) {
				...
				"columns" : [ 
					...
					"orderable" : false,
					"mRender" : function(data, type, row) {
						var action = '<a href="deliveryOrder/' + row.id + '" data-placement="top" title=<spring:message code="tooltip.edit" /> ><img src="${pageContext.request.contextPath}/resources/images/edit1.png"></a>&nbsp;&nbsp;';
						return action;
					}
				},{
					"data" : "deliveryId",
