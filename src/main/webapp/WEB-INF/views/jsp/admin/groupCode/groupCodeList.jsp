...
<title><spring:message code="groupCode.title" /></title>
<meta name="description" content="">
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no">
<sec:authorize access="hasRole('GROUP_CODE_EDIT') or hasRole('ADMIN')" var="canEdit" />
<sec:authorize access="hasRole('ROLE_ADMIN_READONLY')" var="buyerReadOnlyAdmin" />
<spring:message var="groupCodeListDesk" code="application.buyer.groupCode.list" />
<sec:authentication property="principal.languageCode" var="languageCode" />
<script>
zE(function() {
  zE.setHelpCenterSuggestions({ labels: [${groupCodeListDesk}] });
});
</script>
	...
	<div id="page-content">
		<div class="container col-md-12">
			...
			<ol class="breadcrumb">
				...
				<li class="active">
					<spring:message code="groupCode.list" />
				</li>
			</ol>
								...
								<div class="col-xs-12">
									<div class="upload_download_wrapper">
										<div class="right_button">
											<c:url value="/buyer/groupCodeCsv" var="groupCodeCsv" />
											<a href="${groupCodeCsv}">
												<button id="downloadTemplate" class="btn btn-sm btn-success hvr-pop marg-left-10 f-r h-37" data-toggle="tooltip" data-placement="top" data-original-title='<spring:message code="download.group.Code"/>'>
													<span class="glyph-icon icon-separator"> <i class="glyph-icon icon-download"></i></span>
													<span class="button-content"><spring:message code="download.group.Code" /></span>
												</button>
											</a>
											<c:url value="/buyer/downloadGCTemplate" var="downloadGCTemplate" />
											<a href="${downloadGCTemplate}">
											...
											<button class="btn green-btn hvr-pop hvr-rectangle-out2" id="uploadGroupCode">
											...
											<div data-provides="fileinput" class="fileinput hide fileinput-new input-group">
												...
												<span class="input-group-addon btn btn-black btn-file"> 
													...
													<input type="file" data-buttonName="btn-black" id="uploadGroupCodeFile" name="uploadGroupCodeFile" accept="application/vnd.ms-excel">
												</span>
											</div>
										</div>
									</div>
										...
										<table id="tableList" class="data  display table table-bordered noarrow" cellspacing="0" width="100%">
											<thead>
												<tr class="tableHeaderWithSearch">
													...
													<th search-type="text"><spring:message code="label.groupCode.description" /></th>
													...
													<th search-type="select" search-options="statusList"><spring:message code="label.groupCode.status" /></th>
												</tr>
											</thead>
											<tbody>
											</tbody>
										</table>
										<form method="get" action="groupCode">
											<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}" />
											<button type="submit" class="btn btn-plus btn-info top-marginAdminList ${canEdit and !buyerReadOnlyAdmin ? '' : 'disabled' }">
												<spring:message code="groupCode.create" />
											</button>
										</form>
									</div>
...
.scrolableTable_UserList>div.dataTables_wrapper {
	...
	$('document').ready(function() {
					...
					table = $('#tableList').DataTable({
								...
								"searching" : true,
								"ajax" : {
									"url" : getContextPath() + "/buyer/groupCodeListData",
									"data" : function(d) {
									},
								...
								"columns" : [
										{
											...
											"mRender" : function(data, type, row) {
												...
												return '<a href="editGroupCode?id=' + row.id + '"  title=<spring:message code="tooltip.edit" />><img src="${pageContext.request.contextPath}/resources/images/edit1.png"></a>  <a href="#myModal" onClick="javascript:updateLink(\'' + row.id
		...
		function doCancel() {
			var link = $("a#idConfirmDelete");
			link.data('href', link.attr("href"));
			link.attr("href", '${pageContext.request.contextPath}/buyer/deleteGroupCode?id=');
		}
	</script>
		...
		<div class="modal-dialog for-delete-all reminder">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<label><spring:message code="groupCode.delete" /></label>
				</div>
				<div class="modal-footer pad_all_15 float-left width-100 border-top-width-1">
					<a id="idConfirmDelete" class="btn btn-info pull-left ph_btn_small hvr-pop hvr-rectangle-out" href="${pageContext.request.contextPath}/buyer/deleteGroupCode?id=" title=<spring:message code="tooltip.delete" />>
	...
	$('#uploadGroupCode').click(function(e) {
		e.preventDefault();
		$('#uploadGroupCodeFile').trigger("click");

	});
	
	var files = [];
	$(document).on("change", "#uploadGroupCodeFile", function(event) {
		files = event.target.files;
	});
	
	$("#uploadGroupCodeFile").on("change", function() {
		if ($(this).val() == "") {
			return;
		}
		...
		if ($('#uploadGroupCodeFile').val().length == 0) {
		...
		myForm.append("file", $('#uploadGroupCodeFile')[0].files[0]);
		$.ajax({
			url : getContextPath()+ '/buyer/uploadGroupCode',
			data : myForm,
			type : "POST",
