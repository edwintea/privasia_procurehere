	...
	<div class="container col-md-12">
		...
		<ol class="breadcrumb">
			...
			<sec:authorize access="hasRole('SUPPLIER')">
				<li><a
					href="${pageContext.request.contextPath}/supplier/supplierDashboard"">
						<spring:message code="application.dashboard" />
				</a></li>
			</sec:authorize>
			<sec:authorize access="hasRole('OWNER')">
				<li><a
					href="${pageContext.request.contextPath}/owner/ownerDashboard"">
						<spring:message code="application.dashboard" />
				</a></li>
			</sec:authorize>
			<li class="active"><spring:message code="uom.list" /></li>
		</ol>
			...
			<div class="row">
				<div class="col_12">
					<div class="white_box_brd pad_all_15">
						<section class="index_table_block"> <header
							class="form_header"> <form:errors path="*"
							cssClass="error" /> <c:if test="${not empty errors}">
						...
						<div class="row">
							<div class="col-xs-12">
								...
								<%-- 										<c:url value="/admin/uom/uomTemplate" var="uomTemplate" /> --%>
								<%-- 										<a href="${uomTemplate}"> --%>
								<!-- 											<button -->
								<!-- 												class="btnUmoList btn btn-default hvr-pop hvr-rectangle-out3" -->
								<!-- 												id="downloadTemplate" data-target="#myModal10" -->
								<!-- 												data-toggle="modal"> -->
								...
								<!-- 									</div> -->
								<!-- 								</div> -->
								<c:url value="/admin/uom/exportUomCsvReport"
									var="downloadCsvReport" />
								<form:form action="${downloadCsvReport}" method="post"
									id="exportUomForm" ModelAttribute="uomPojo">
									...
									<div class="ph_tabel_wrapper scrolableTable_UserList">
										<table id="tableList"
											class=" display table table-bordered noarrow" cellspacing="0"
											width="100%">
											<thead>
												<tr class="tableHeaderWithSearch">
													...
													<th search-type="text"><spring:message
															code="label.uom" /></th>
													<th search-type="text"><spring:message
															code="application.description" /></th>
													<th search-type="text"><spring:message
										...
										<spring:url value="/admin/uom/uom" var="createUrl"
											htmlEscape="true" />
										<a href="${canEdit ? createUrl : '#'}"
											class="btn btn-info top-marginAdminList ${canEdit ? '' : 'disableEve'} ${!buyerReadOnlyAdmin ? '':'disabled'}">
											<spring:message code="uom.create" />
										</a>
									</div>
								</form:form>
...
<script type="text/javascript">
	$('document').ready(function() {
		...
		var table = $('#tableList').DataTable({
			...
			"searching" : true,
			"ajax" : {
				"url" : getContextPath() + "/admin/uom/uomListData",
				"data" : function(d) {
				},
			...
			"columns" : [{
				...
				"mRender" : function(data, type, row) {
					...
					return '<a href="editUom?id=' + row.id + '" title=<spring:message code="tooltip.edit" />><img src="${pageContext.request.contextPath}/resources/images/edit1.png"></a>  <a href="#myModal" onClick="javascript:updateLink(\'' + row.id + '\');"  title=<spring:message code="tooltip.delete" /> role="button" class="${!buyerReadOnlyAdmin ? "":"disabled"}" data-toggle="modal">' + tImg + '</a>';
	...
	<div class="modal-dialog for-delete-all reminder">
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<label> <spring:message code="uom.delete" />
				</label>
			</div>
			<div
				...
				<a id="idConfirmDelete"
					...
					href="${pageContext.request.contextPath}/admin/uom/deleteUom?id="
