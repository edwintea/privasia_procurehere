...
<div class="panel sum-accord upload_download_wrapper clearfix marg-top-10 event_info">
	<div class="panel-title list-pad-0" style="margin-bottom: 0px;">
	...
	<div id="collapseAudit" class="panel-collapse collapse in">
		<div class="col-md-12 col-sm-12 col-xs-12 in">
				...
				<table class="tabaccor padding-none-td table" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-bottom:0px;">
					<thead>
						<tr>
							...
							<th class="align-left width_300"><spring:message code="product.description" /></th>
							<th class="align-left width_200"><spring:message code="application.snapshot" /></th>
						</tr>
					</thead>
					<tbody>
						<c:forEach items="${contractAuditList}" var="audit">
							<tr>
								...
								<td class="align-left width_200_fix">${audit.action.value}</td>
								<td class="align-left width_300 " >${audit.description}</td>
								<td class="align-left width_100_fix">
	 								<c:if test="${audit.hasSnapshot}">
										<fmt:formatDate value="${audit.actionDate}" pattern="ddMMyyyyHHmm" timeZone="<%=request.getSession().getAttribute(\"timeZone\")%>" var="actionDate" />
										<form:form method="GET">
											<c:url var="download" value="/buyer/downloadAuditFile/${audit.id}/${audit.action}/${actionDate}" />
											<a class="downloadSnapshot" href="${download}" title="Download Snapshot"><img src="${pageContext.request.contextPath}/resources/images/download.png" alt="download"></a>
										</form:form>
									</c:if>
								</td>
							</tr>
