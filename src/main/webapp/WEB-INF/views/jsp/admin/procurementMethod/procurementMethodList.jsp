	...
	<div id="page-content">
		<div class="container col-md-12">
			...
			<ol class="breadcrumb">
				...
				<li class="active">
					<spring:message code="procurement.method.list" />
				</li>
			</ol>
								...
								<div class="col-xs-12">
									<div class="upload_download_wrapper">
										<div class="right_button">
											<c:url value="/buyer/procurementMethodCsv" var="procurementMethodCsv" />
											<a href="${procurementMethodCsv}">
												<button class="btn btn-sm btn-success hvr-pop marg-left-10 f-r h-37" id="downloadCsvFile" data-target="#myModal10" data-toggle="modal">
													<span class="glyph-icon icon-separator"> <i class="glyph-icon icon-download"></i></span>
													<span class="button-content"><spring:message code="download.procurement.method.csv.file" /></span>
												</button>
											</a>
											<c:url value="/buyer/downloadPMTemplate" var="downloadPMTemplate" />
											<a href="${downloadPMTemplate}">
												<button class="btn btn-default hvr-pop hvr-rectangle-out2" id="downloadTemplate" data-target="#myModal10" data-toggle="modal">
													<i class="excel_icon"></i><spring:message code="application.download.excel.button" />
												</button>
											</a>
											<button class="btn green-btn hvr-pop hvr-rectangle-out3" id="uploadProcurementMethod">
											...
											<div data-provides="fileinput" class="fileinput hide fileinput-new input-group">
												...
												<span class="input-group-addon btn btn-black btn-file">
													<span class="fileinput-new"> <spring:message code="event.document.selectfile" /> </span>
													<span class="fileinput-exists"> <spring:message code="event.document.change" /> </span>
													<input type="file" data-buttonName="btn-black" id="uploadProcurementMethodFile" name="uploadProcurementMethodFile" accept="application/vnd.ms-excel">
												</span>
										...
										<table id="tableList" class="data  display table table-bordered noarrow" cellspacing="0" width="100%">
											<thead>
												<tr class="tableHeaderWithSearch">
													...
													<th search-type="text"><spring:message code="label.procurement.code"/></th>
													<th search-type="text"><spring:message code="label.procurement.method" /></th>
...
.scrolableTable_UserList>div.dataTables_wrapper {
	...
	$('document').ready(
								...
								"columns" : [
										{
											...
											"mRender" : function(data, type, row) {
												...
												return '<a href="editProcurementMethod?id=' + row.id + '"  title=<spring:message code="tooltip.edit" />><img src="${pageContext.request.contextPath}/resources/images/edit1.png"></a>  <a href="#myModal" onClick="javascript:updateLink(\'' + row.id
														+ '\');" title=<spring:message code="tooltip.delete" /> role="button" class="${!buyerReadOnlyAdmin ? "":"disabled"}" data-toggle="modal">' + tImg + '</a>';
											}
										}, {
											"data" : "procurementMethodCode",
										    "defaultContent" : ""
									    }, {
		...
		function doCancel() {
			var link = $("a#idConfirmDelete");
			link.data('href', link.attr("href"));
			link.attr("href", '${pageContext.request.contextPath}/buyer/deleteProcurementMethod?id=');
		}
	</script>
		...
		<div class="modal-dialog for-delete-all reminder">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<label><spring:message code="procurement.method.delete" /></label>
				</div>
				<div class="modal-footer pad_all_15 float-left width-100 border-top-width-1">
					<a id="idConfirmDelete" class="btn btn-info pull-left ph_btn_small hvr-pop hvr-rectangle-out" href="${pageContext.request.contextPath}/buyer/deleteProcurementMethod?id=" title=<spring:message code="tooltip.delete" />>
	...
	<script>
		...
		$('#uploadProcurementMethod').click(function(e) {
			e.preventDefault();
			$('#uploadProcurementMethodFile').trigger("click");

		});

		var files = [];
		$(document).on("change", "#uploadProcurementMethodFile", function(event) {
			files = event.target.files;
		});

		$("#uploadProcurementMethodFile").on("change", function() {
			if ($(this).val() == "") {
				return;
			}
			...
			if ($('#uploadProcurementMethodFile').val().length == 0) {
			...
			myForm.append("file", $('#uploadProcurementMethodFile')[0].files[0]);
			$.ajax({
				url : getContextPath()+ '/buyer/uploadProcurementMethod',
				data : myForm,
				type : "POST",
