...
<div id="page-content" view-name="declarationList">
						...
						<div class="row">
							<div class="col-xs-12">
								<div class="ph_tabel_wrapper scrolableTable_UserList">
									<table id="tableList" class=" display table table-bordered noarrow" cellspacing="0" width="100%">
										<thead>
											<tr class="tableHeaderWithSearch">
												...
												<th search-type="text"><spring:message code="declaration.title" /></th>
												<th search-type="select"  search-options="declarationTypeList"><spring:message code="declaration.type" /></th>
									...
									<spring:url value="/buyer/declaration" var="createUrl" htmlEscape="true" />
									<c:if test="${canEdit}">
									<a href="${createUrl}" class="btn btn-info top-marginAdminList"> <spring:message code="declaration.create" />
									</a>
									</c:if>
								</div>
	...
	<div class="modal-dialog for-delete-all reminder">
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<label> <spring:message code="declaration.delete" />
				</label>
			</div>
			<div class="modal-footer pad_all_15 float-left width-100 border-top-width-1">
				<a id="idConfirmDelete" class="btn btn-info ph_btn_small pull-left hvr-pop hvr-rectangle-out" href="${pageContext.request.contextPath}/buyer/deleteDeclaration?id=" title="Delete"> <spring:message code="application.delete" />
				</a>
...
$('document').ready(function(){
	var table=$('#tableList').DataTable({
		...
		"searching" : true,
		"ajax":{
			"url":getContextPath()+"/buyer/declarationListData",
			"data":function(d){
			},
		...
		"columns":[{
				...
				"mRender":function(data,type,row){
					...
					ret += '<a href="editDeclaration?id=' + row.id +'" title="Edit"><img src="${pageContext.request.contextPath}/resources/images/edit1.png"></a>'; 
					if (canEdit() == "true" ) {
	...
	$('#tableList thead tr:nth-child(1) th').each(function(i) {
		...
		if (!(title == "Actions") && $(this).attr('search-type') != '') {
			if ($(this).attr('search-type') == 'select') {
				...
				if (optionsType == 'declarationTypeList') {
					<c:forEach items="${declarationTypeList}" var="item">
					htmlSearch += '<option value="${item}">${item}</option>';
					</c:forEach>
				}
...
function doCancel() {
	var link = $("a#idConfirmDelete");
	link.data('href', link.attr("href"));
	link.attr("href", '${pageContext.request.contextPath}/buyer/deleteDeclaration?id=');
}
</script>