										...
										<div class="right_button">
											...
											<a href="${productCategoryTemplate}">
												<button class="btn btn-default hvr-pop hvr-rectangle-out3" id="downloadTemplate" data-target="#myModal10" data-toggle="modal">
													<i class="excel_icon"></i>Download Excel Template
												</button>
											</a>
											<button class="btn green-btn hvr-pop hvr-rectangle-out2" id="uploadProductCategory">
												<i class="upload_icon"></i> Upload List of Item
											</button>
									...
									<table id="tableList" class=" display table table-bordered noarrow" cellspacing="0" width="100%">
										<thead>
											<tr class="tableHeaderWithSearch">
												...
												<th search-type="text"><spring:message code="Rfsz.code" /></th>
												<th search-type="text"><spring:message code="Rfsz.name" /></th>
												...
												<th search-type="select" search-options="statusList"><spring:message code="Rfsz.status" /></th>
											</tr>
										</thead>
										<tbody>
										</tbody>
									</table>
									<form method="get" action="rfsCategory">
									<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}" />
										<button type="submit" class="btn btn-plus btn-info top-marginAdminList ${canEdit and !buyerReadOnlyAdmin ? '' : 'disabled'}" >
											<spring:message code="rfsz.create" />
										</button>
									</form>
								</div>
...
var table = '';
	...
	$('document').ready(function() {
		...
		table = $('#tableList').DataTable({
			...
			"searching" : true,
			"ajax" : {
				"url" : getContextPath() + "/buyer/rfsCategoryData",
				"data" : function(d) {
				},
			...
			"columns" : [ {
				...
				"render" : function(data, type, row) {
					var ret = '<a href="rfsCategoryEdit?id=' + row.id + '"  title=<spring:message code="tooltip.edit" />><img src="${pageContext.request.contextPath}/resources/images/edit1.png"></a>';
...
<div class="modal fade" id="myModal" role="dialog">
	<div class="modal-dialog for-delete-all reminder">
		...
		<div class="modal-content">
			...
			<label>
					<spring:message code="Rfsz.delete" />
				</label>
			</div>
			<div class="modal-footer pad_all_15 float-left width-100 border-top-width-1">
				<a id="idConfirmDelete" class="btn pull-left btn-info ph_btn_small hvr-pop hvr-rectangle-out" href="${pageContext.request.contextPath}/buyer/rfsCategoryDelete?id=" title=<spring:message code="tooltip.delete" />>
