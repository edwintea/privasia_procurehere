...
<div id="apprTab" class="pad_all_15 collapse in float-leftwidth-100position-relative">
	<div class="dynamic-approval form-group">
		...
		<!-- For LOOP -->
		<c:forEach items="${sourceForm.sourcingFormApproval}" var="approval" varStatus="status">
			<div id="new-approval-${status.index + 1}" class="row new-approval">
				...
				<div class="col-md-4 col-sm-6 col-xs-12 col-lg-3 ${assignedCount > 0 ? 'disabled':''}">
					<form:select data-validation="required" id="multipleSelectExample-${status.index}" path="sourcingFormApproval[${status.index}].approvalUsers" cssClass="tagTypeMultiSelect" data-placeholder="Approvers" multiple="multiple">
				...
				<div class="col-sm-2 col-md-2 col-xs-2 pad0">
					<div class="btn-address-field">
						...
						<ul class="dropdown-menu dropup">
							<li><a href="javascript:void(0);" class="small" tabIndex="-1"> <form:checkbox path="sourcingFormApproval[${status.index}].approvalType" value="OR" cssClass="approval_condition" label="Any"></form:checkbox>
							</a></li>
							<li><a href="javascript:void(0);" class="small " tabIndex="-1"> <form:checkbox path="sourcingFormApproval[${status.index}].approvalType" value="AND" cssClass="approval_condition" label="All"></form:checkbox>
							</a></li>
						</ul>
...
<script type="text/JavaScript">
	var index;
	index = ${fn:length(sourceForm.sourcingFormApproval) + 1};
	$(document).ready(function() {
		...
		$("#addSelect").click(function(e) {
			...
			template += '<select data-validation="required" id="multipleSelectExample-'+(index - 1)+'" name="sourcingFormApproval[' + (index - 1) + '].approvalUsers" class="tagTypeMultiSelect" data-placeholder="Approvers" multiple>';
