	...
	<div class="approVAlRouteBox " style="max-height: unset !important; overflow-y: unset !important; overflow-x: unset !important;">
		<c:forEach items="${supplierForm.approvals}" var="approval" varStatus="status">
			<div id="editnew-approval-${status.index + 1}" class="row new-approval editnew-approval ">
				...
				<form:hidden class="approval_id_hidden editapproval_id_hidden" path="approvals[${status.index}].id" />
				...
				<div class="col-md-7 col-sm-7 col-xs-8 approval-chosen-multiselect approvalsLevelSelect ${approval.done ? 'disabled':''}">
					<span class="dropUp"> <form:select  data-validation="required"  id="multipleSelectExample-${status.index}" path="approvals[${status.index}].approvalUsers" cssClass="edittagTypeMultiSelect tagTypeMultiSelect user-list-all chosen-select  ${approval.done  ? 'disabled':''}" data-placeholder="${approversplace}" multiple="multiple">
				...
				<div class="col-sm-6 col-md-3 col-xs-4">
					<div class="btn-address-field pt-0">
						...
						<ul class="dropdown-menu dropup">
							<li>
								<a href="javascript:void(0);" class="small editsmall" tabIndex="-1"> <form:checkbox path="approvals[${status.index}].approvalType" value="OR" cssClass="approval_condition editapproval_condition" label="Any"></form:checkbox>
								</a>
							</li>
							<li>
								<a href="javascript:void(0);" class="small editsmall" tabIndex="-1"> <form:checkbox path="approvals[${status.index}].approvalType" value="AND" cssClass="approval_condition editapproval_condition" label="All"></form:checkbox>
								</a>
							</li>
						</ul>
...
<script type="text/JavaScript">
	var indexx;
	indexx = ${fn:length(supplierForm.approvals) + 1};
	$(document).ready(function() {
		...
		$("#addNewSelect").click(function(e) {
			...
			template += '<input type="hidden" class="approval_id_hidden editapproval_id_hidden" name="approvals[' + (indexx - 1) + '].id" value="" />';
			template += '<div class="col-md-2 col-sm-3"><label class="level">Level ' + indexx + '</label></div>';
			template += '<div class="col-md-7 col-sm-7" id="sel">';
			template += '<span class="dropUp"><select data-validation="required" autocomplete="off" id="multipleSelectExample-'+(indexx - 1)+'" name="approvals[' + (indexx - 1) + '].approvalUsers" class="edittagTypeMultiSelect user-list-all chosen-select tagTypeMultiSelect" data-placeholder="Approvers" multiple>';
			...
			template += '<li><a href="javascript:void(0);" class="small editsmall"  tabIndex="-1"><input name="approvals[' + (indexx - 1) + '].approvalType" checked="checked" value="OR" class="approval_condition editapproval_condition" type="checkbox"/><label>Any</label></a></li>';
			template += '<li><a href="javascript:void(0);" class="small editsmall" tabIndex="-1"><input name="approvals[' + (indexx - 1) + '].approvalType" value="AND" class="approval_condition editapproval_condition"  type="checkbox"/><label>All</label></a></li>	</ul>';
		...
		$(document).delegate('.editremoveApproval', 'click', function(e) {
			...
			$(".editnew-approval").each(function(i, v) {
				...
				$(this).find("input[name='approvals[" + (i) + "].approvalType']").each(function(){//
					$(this).attr('name','approvals[' +(i-1) + '].approvalType');
				...
				$(this).find("input[name='approvals[" + (i) + "].approvalUsers']").each(function(){//
					$(this).attr("name",'approvals[' +(i-1) + '].approvalUsers');
				}) //select name reindex
