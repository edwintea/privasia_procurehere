...
<div id="page-content" view-name="rfxTemplate">
	<div class="container col-md-12">
		...
		<ol class="breadcrumb">
			...
			<li class="active"><spring:message code="rfxTemplate.list" /></li>
		</ol>
		<div class="Section-title title_border gray-bg">
			<h2 class="trans-cap manage_icon"><spring:message code="rfxTemplate.list" /></h2>
		</div>
								...
								<div class="col-xs-12">
									<div class="form-group col-md-12 bordered-row">
										<div class="ph_tabel_wrapper scrolableTable_list">
											<table id="tableList" class=" display table table-bordered noarrow" cellspacing="0" width="100%">
												<thead>
													<tr class="tableHeaderWithSearch">
														...
														<th search-type="text"><spring:message code="rfxTemplate.templateDescription" /></th>
											...
											<spring:url value="/buyer/rfxTemplate/rfxTemplate" var="createUrl" htmlEscape="true" />
											<sec:authorize access="hasRole('EVENT_TEMPLATE_EDIT') or hasRole('ADMIN')" var="canEdit" />
												<a href="${canEdit ? createUrl : '#'}" class="btn btn-info top-marginAdminList ${canEdit ? '' : 'disableEve'} ${!buyerReadOnlyAdmin ? '':'disabled'}"> <spring:message code="rfxTemplate.create" /></a>
										</div>
...
<script type="text/javascript">
	$('document').ready(function() {
		...
		var table = $('#tableList').DataTable({
			...
			"searching" : true,
			"ajax" : {
				"url" : getContextPath() + "/buyer/rfxTemplate/rfxTemplateData",
				"data" : function(d) {
					//var table = $('#tableList').DataTable()
			...
			"columns" : [ {
				...
				"render" : function(data, type, row) {
					var ret = '<a href="${pageContext.request.contextPath}/buyer/rfxTemplate/editRfxTemplate?id=' + row.id + '"  title=<spring:message code="application.edit"/>><img src="${pageContext.request.contextPath}/resources/images/edit1.png"></a>';
					ret += '<a href="#myModal" onClick="javascript:updateLink(\'' + row.id + '\');" title=<spring:message code="application.delete" /> role="button" class="${!buyerReadOnlyAdmin ? "":"disabled"}" data-toggle="modal">';
		...
		$('#tableList thead tr:nth-child(1) th').each(function(i) {
			...
			if (!(title == "Actions") && $(this).attr('search-type') != '') {
				if($(this).attr('search-type') == 'select'){
					...
					if(optionsType == 'rfxTypeList'){
						<c:forEach items="${rfxTypeList}" var="item">
	...
	<div class="modal-dialog for-delete-all reminder">
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<label><spring:message code="rfxTemplate.delete" /></label>
			</div>
			<div class="modal-footer pad_all_15 float-left width-100 border-top-width-1">
				<a id="idConfirmDelete" class="btn btn-info ph_btn_small hvr-pop hvr-rectangle-out" href="${pageContext.request.contextPath}/buyer/rfxTemplate/deleteRfxTemplate?id=" title=<spring:message code="application.delete" />> <spring:message
