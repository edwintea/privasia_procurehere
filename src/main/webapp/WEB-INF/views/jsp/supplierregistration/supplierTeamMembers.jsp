				...
				<div class="mem-tab">
					...
					<table cellpading="0" cellspacing="0" width="100%" id="appUsersList" class="table-hover table">
						<c:forEach items="${teamMembers}" var="teamMember">
							<tr data-username="${teamMember.user.name}" approver-id="${teamMember.user.id}">
								<td class="width_50_fix ">
									<!--	<img src='<c:url value="/resources/images/profile_setting_image.png"/>' alt="profile " /> -->
								</td>
								<td>${teamMember.user.name}
									<br> <span>${teamMember.user.loginId}</span>
								</td>
								<td class="edit-drop">
									<div class="advancee_menu">
										<div class="adm_box">
											<a class="adm_menu_link dropdown-toggle" data-toggle="dropdown">
												<i class="glyphicon ${teamMember.teamMemberType=='Editor'?'glyphicon-pencil':'glyphicon-eye-open'} " aria-hidden="true "
												
														title="${teamMember.teamMemberType=='Editor'?'Editor':'Viewer'}" 
												></i>
											</a>
											<ul class="dropdown-menu dropup">
												<li>
													<a href="javascript:void(0);" class="small" tabIndex="-1">
														<input data-uid="${teamMember.user.id}" id="${teamMember.user.id}-Editor" ${teamMember.teamMemberType=='Editor' ? 'checked' : ''} value="Editor" class="access_check" type="checkbox" />
														&nbsp;<spring:message code="eventsummary.checkbox.editor" />
													</a>
												</li>
												<li>
													<a href="javascript:void(0);" class="small " tabIndex="-1">
														<input id="${teamMember.user.id}-Viewer" ${teamMember.teamMemberType=='Editor'?'': 'checked'} value="Viewer" data-uid="${teamMember.user.id}" class="access_check" type="checkbox" />
														&nbsp;<spring:message code="eventsummary.checkbox.viewer" />
													</a>
												</li>
											</ul>
								...
								<td>
									<div class="cqa_del">
										<a href="#" list-type="Team Member" class="removeApproversList" data-toggle="dropdown" title='<spring:message code="tooltip.delete" />'></a>
									</div>
								</td>
							</tr>
						</c:forEach>
	...
	<div class="float-left width100 pad_all_15 white-bg">
		...
		<div class="row">
			<p class="col-md-12 marg-bottom-20 approverInfoBlock">
				<spring:message code="event.confirm.to.remove" /> <span></span> <spring:message code="application.from" /> <span></span> <spring:message code="application.list" /> ?
			</p>
		</div>
		<div class="row">
			<div class="col-md-12">
