...
<div>
	<div
		class="col-sm-5 col-md-5 col-xs-6 ${!empty readOnlyTeamMember and readOnlyTeamMember ?'disabled':''  }">
		<div class="col-md-12">
				...
				<div class="group">
					<div class="input-group mrg15T mrg15B">
						<select id="TeamMemberList" class="user-list-normal chosen-select"
							name="approverList1" cssClass="form-control chosen-select">
							<option value=""><spring:message
									code="prsummary.select.team.member" /></option>
							<c:forEach items="${userTeamMemberList}" var="usr">
								<c:if test="${usr.id == '-1' }">
									<option value="-1" disabled="true">${usr.name}</option>
								</c:if>
								<c:if test="${usr.id != '-1' }">
		...
		<div class="col-md-12 col-sm-12 col-xs-12">
			<div class="mem-tab">
				<table cellpading="0" cellspacing="0" width="100%" id="appUsersList"
					...
					<c:forEach items="${sourcingFormRequest.sourcingFormTeamMember}"
						var="teamMembers">
