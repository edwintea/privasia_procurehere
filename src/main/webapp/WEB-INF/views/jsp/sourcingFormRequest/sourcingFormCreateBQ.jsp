					...
					<div class="margin-bottom-10">
						<div class="meeting2-heading">
							...
							<button class="pull-right btn ph_btn_midium btn-info toggleCreateBq hvr-pop hvr-rectangle-out editBqItem" type="button" data-toggle="tooltip"
								data-id="${sourcingFormRequestBq.id}" data-original-title='<spring:message code="edit.bq.placeholder" />' style="margin-right: 20px; margin-top: 7px; height: 30px; line-height: 1;"><spring:message code="event.edit.bq" />
								</button>
						</div>
						<div class="import-supplier-inner-first-new pad_all_15 global-list form-middle" id="bqDisplay" style="display: none;">
							<form id="bqFormAddEdit" class="form-horizontal bordered-row has-validation-callback" action="" method="post">
								...
								<input id="editBqName" name="editBqName" value="${sourcingFormRequestBq.name}" type="hidden">
								<input id="editBqDesc" name="editBqDesc" value="${sourcingFormRequestBq.description}" type="hidden">
					...
					<div class="column_button_bar">
						...
						<div class="pull-right">
							<div class="row" style="margin-right: 0px;">
								<input name="bqItemSearch" type="text" id="bqItemSearch" placeholder='<spring:message code="bq.search.itemname.placeholder" />' class="form-control" />
							</div>
						</div>
					</div>
					...
					<div class="row">
						...
						<div class="col-md-6 col-sm-6 col-xs-12">
							<div class="form-group unitPriceClass">
								...
								<c:choose>
									<c:when test="${sourcingFormRequest.decimal==1}">
										<c:set var="decimalSet" value="0,0.0"></c:set>
									</c:when>
									<c:when test="${sourcingFormRequest.decimal==2}">
										<c:set var="decimalSet" value="0,0.00"></c:set>
									</c:when>
									<c:when test="${sourcingFormRequest.decimal==3}">
										<c:set var="decimalSet" value="0,0.000"></c:set>
									</c:when>
									<c:when test="${sourcingFormRequest.decimal==4}">
										<c:set var="decimalSet" value="0,0.0000"></c:set>
									</c:when>
								</c:choose>
								<input type="text" data-validation="custom required" class="form-control" disabled="true" name="unitPrice" id="itemUnitPrice" placeholder="${unitpricing}"
									data-validation-ignore="," data-validation-regexp="^\d{0,10}(\.\d{1,${sourcingFormRequest.decimal}})?$"
									data-validation-error-msg="Only numbers allowed, length should be less then 10 and after decimal ${sourcingFormRequest.decimal} digits allowed"
									data-sanitize="numberFormat" data-sanitize-number-format="${decimalSet}">
							</div>
						</div>
					</div>
					<div class="row">
						<div class="col-md-6 col-sm-6 col-xs-12">
							<div class="form-group">
								<label><spring:message code="label.rftbq.th.quantity.only" /></label>
								<c:choose>
									<c:when test="${sourcingFormRequest.decimal==1}">
										<c:set var="decimalSet" value="0,0.0"></c:set>
									</c:when>
								...
								<input type="text" data-validation="custom required" class="form-control"  name="quantity" id="itemQuantity" placeholder="${quantity}"
									data-validation-ignore="," data-validation-regexp="^\d{0,10}(\.\d{1,${sourcingFormRequest.decimal}})?$"
									data-validation-error-msg="Only numbers allowed, length should be less then 10 and after decimal ${sourcingFormRequest.decimal} digits allowed"
									data-sanitize="numberFormat" data-sanitize-number-format="${decimalSet}">
							</div>
						...
						<div class="col-md-6 col-sm-6 col-xs-12">
							<div class="form-group unitBudgetClass">
								<label class="marg-bottom-10"><spring:message code="product.unit.budget.price" /></label>
								<c:choose>
									<c:when test="${sourcingFormRequest.decimal==1}">
										<c:set var="decimalSet" value="0,0.0"></c:set>
									</c:when>
								...
								<input type="text" data-validation="custom " class="form-control"  name="unitBudgetPrice" id="unitBudgetPrice" placeholder="${unitBudgetpricing}"
									data-validation-ignore="," data-validation-regexp="^\d{0,10}(\.\d{1,${sourcingFormRequest.decimal}})?$"
									data-validation-error-msg="Only numbers allowed, length should be less then 10 and after decimal ${sourcingFormRequest.decimal} digits allowed"
									data-sanitize="numberFormat" data-sanitize-number-format="${decimalSet}">
							</div>
					...
					<div class="row">
						<div class="col-md-6 col-sm-6 col-xs-12">
							<div class="form-group">
								...
								<textarea class="form-control" id="itemDesc" name="itemDescription" placeholder='<spring:message code="event.bq.enter.description"/>' maxlength="1000"></textarea>
								<span class="sky-blue"><spring:message code="dashboard.valid.max2.characters" /></span>
							</div>
						</div>					
			...
			<div class="table_fix_header">
				<table class="header_table header bq_font" width="100%" cellpadding="0" cellspacing="0" id="table_id">
					<tr>
						...
						<th class="hed_5 width_200 width_200_fix align-right "><spring:message code="label.rftbq.th.quantity.only" /></th>
						<th class="hed_5 width_200 width_200_fix align-right "><spring:message code="label.rftbq.th.unitprice" /></th>
						<th class="hed_5 width_200 width_200_fix align-right "><spring:message code="label.rftbq.th.unitbudgetprice" /></th>
						...
						<c:if test="${not empty sourcingFormRequestBq.field2Label}">
							<th class="hed_4 width_200 width_200_fix extraFieldHeaders">${sourcingFormRequestBq.field2Label}</th>
						</c:if>
						<c:if test="${not empty sourcingFormRequestBq.field3Label}">
							<th class="hed_4 width_200 width_200_fix extraFieldHeaders">${sourcingFormRequestBq.field3Label}</th>
						</c:if>
						<c:if test="${not empty sourcingFormRequestBq.field4Label}">
							<th class="hed_4 width_200 width_200_fix extraFieldHeaders">${sourcingFormRequestBq.field4Label}</th>
						</c:if>
						<c:if test="${not empty sourcingFormRequestBq.field5Label}">
							<th class="hed_4 width_200 width_200_fix extraFieldHeaders">${sourcingFormRequestBq.field5Label}</th>
						</c:if>
						<c:if test="${not empty sourcingFormRequestBq.field6Label}">
							<th class="hed_4 width_200 width_200_fix extraFieldHeaders">${sourcingFormRequestBq.field6Label}</th>
						</c:if>
						<c:if test="${not empty sourcingFormRequestBq.field7Label}">
							<th class="hed_4 width_200 width_200_fix extraFieldHeaders">${sourcingFormRequestBq.field7Label}</th>
						</c:if>
						<c:if test="${not empty sourcingFormRequestBq.field8Label}">
							<th class="hed_4 width_200 width_200_fix extraFieldHeaders">${sourcingFormRequestBq.field8Label}</th>
						</c:if>
						<c:if test="${not empty sourcingFormRequestBq.field9Label}">
							<th class="hed_4 width_200 width_200_fix extraFieldHeaders">${sourcingFormRequestBq.field9Label}</th>
						</c:if>
						<c:if test="${not empty sourcingFormRequestBq.field10Label}">
							<th class="hed_4 width_200 width_200_fix extraFieldHeaders">${sourcingFormRequestBq.field10Label}</th>
						</c:if>						
														...
														<tr class="sub_item item_${bqchild.level}_${bqchild.order}" data-id="${bqchild.id}" data-priceType="${bqchild.priceType}" data-parent="${bq.id}">
															...
															<td class="width_200 width_200_fix align-left">
																<span class="item_name itemNameD">
																	<c:if test="${bqchild.priceType == 'TRADE_IN_PRICE'}">
																		<span class="bs-label label-info"><spring:message code="eventsummary.bq.trade.price" /></span>&nbsp;
																			</c:if>
																	<c:if test="${bqchild.priceType == 'BUYER_FIXED_PRICE'}">
															...
															<td class="width_200 width_200_fix align-right">
																<fmt:formatNumber type="number" minFractionDigits="${sourcingFormRequest.decimal}" maxFractionDigits="${sourcingFormRequest.decimal}" value="${bqchild.quantity}" />
															</td>
															<td class="width_200 width_200_fix align-right">
																<fmt:formatNumber type="number" minFractionDigits="${sourcingFormRequest.decimal}" maxFractionDigits="${sourcingFormRequest.decimal}" value="${bqchild.unitPrice}" />
															</td>
															<td class="width_200 width_200_fix align-right">
																<fmt:formatNumber type="number" minFractionDigits="${sourcingFormRequest.decimal}" maxFractionDigits="${sourcingFormRequest.decimal}" value="${bqchild.unitBudgetPrice}" />
															</td>
		...
		<div class="row">
			<div class="col-md-12">
				<div class="table_f_action_btn">
					<form class="bordered-row" id="submitPriviousForm" method="post" style="float: left;" action="${pageContext.request.contextPath}/buyer/sourcingBqListPrevious">
...
<script type="text/javascript" src="<c:url value="/resources/js/view/sourcingRequestBQ.js"/>"></script>
<script>
