...
<div class="dynamic-suspApproval col-md-10">
	<spring:message code="approval.search.user" var="approversplace" />
		<div class="row col-md-8 pad_all_20">
			<div class="setleft">
				<spring:message code="enable.performance.evaluation.approval" var="perfEvalApproval" />
				<form:checkbox path="enablePerformanceEvaluationApproval" id="enablePerformanceEvaluationApproval" class="custom-checkbox" disabled="false" title="" label="${perfEvalApproval}" />
			</div>
		</div>
	<div class="row cloneready marg-bottom-10 pad_all_20">
		<div class="col-md-9">
			<button type="button" class="btn btn-plus btn-info hvr-pop" id="addAppLevel" data-placement="top">
	...
	<div class="approvalSuspRouteBox " style="max-height: unset !important; overflow-y: unset !important; overflow-x: unset !important;">
		<c:forEach items="${evaluatorUser.evaluationApprovals}" var="approval" varStatus="status">
			<div id="new-suspApproval-${status.index + 1}" class="row new-suspApproval">
				<c:if test="${approval.active}">
					<c:forEach items="${evaluatorUser.evaluationApprovals[status.index].approvalUsers}" var="usr">
						<c:if test="${usr.approvalStatus != 'PENDING' }">
							<c:set var="doneUsers" value="${doneUsers.concat(\"'\").concat(usr.user.id).concat(\"',\") }" />
						</c:if>
					</c:forEach>
				</c:if>
				<form:hidden class="approval_id_hidden" path="evaluationApprovals[${status.index}].id" />
				<div class="col-md-2 col-sm-3 col-xs-12">
					<label class="level"><spring:message code="rfi.createrfi.level" /> ${status.index + 1}</label>
				</div>
				<div class="col-md-7 col-sm-7 col-xs-8 approval-chosen-multiselect ${ approval.done ? 'disabled':''} suspApprvl">
					<span class="dropUp appr-DP">
					<form:select data-validation="required" id="suspensionMultipleSelect-${status.index}" path="evaluationApprovals[${status.index}].approvalUsers" cssClass="tagTypeMultiSelect user-list-all chosen-select  ${approval.done || (!empty rfxTemplate ? rfxTemplate.readOnlySuspendApproval : false) ? 'disabled':''}"  data-placeholder="${approversplace}" multiple="multiple">
						<c:forEach items="${evalApprvlUserList}" var="usr" >
							<c:if test="${usr.id == '-1' }">
				...
				<div class="col-sm-6 col-md-3 col-xs-4">
					<div class="btn-address-field pt-0 apprvlType">
						<button type="button" class="btn marg-mins ${approval.approvalType == 'OR' ? 'btn-success' : 'btn-warning'} dropdown-toggle ${ (approval.active || approval.done) ? 'disabled':''}" id="appType" data-toggle="dropdown">
							<i class="fa ${approval.approvalType == 'OR' ? 'fa-user' : 'fa-users'}" aria-hidden="true"></i>
						</button>
						<ul class="dropdown-menu dropup">
							<li><a href="javascript:void(0);" class="small" tabIndex="-1"> <form:checkbox path="evaluationApprovals[${status.index}].approvalType" value="OR" cssClass="approval_condition" label="Any"></form:checkbox>
							</a></li>
							<li><a href="javascript:void(0);" class="small " tabIndex="-1"> <form:checkbox path="evaluationApprovals[${status.index}].approvalType" value="AND" cssClass="approval_condition" label="All"></form:checkbox>
							</a></li>
						</ul>
						<button class="btn btn-plus btn-info hvr-pop hvr-rectangle-out suspRemoveApproval ${approval.active || approval.done ? 'disabled':''}">
...
<script type="text/JavaScript">
	var cnt;
	cnt = ${fn:length(evaluatorUser.evaluationApprovals) + 1};
	...
	//	document.getElementById("addSelect").addEventListener("click", function(e) {
		$("#addAppLevel").click(function(e) {
			e.preventDefault();
			var template = '<div id="new-suspApproval-' + cnt + '" class="row new-suspApproval">';
			template += '<input type="hidden" class="approval_id_hidden" name="evaluationApprovals[' + (cnt - 1) + '].id" value="" />';
			...
			template += '<span class="dropUp"><select data-validation="required" id="suspensionMultipleSelect-'+(cnt - 1)+'" name="evaluationApprovals[' + (cnt - 1) + '].approvalUsers" class="tagTypeMultiSelect chosen-select user-list-all" data-placeholder="${approversplace}" multiple onclick="autoSave()">';
			<c:forEach items="${evalAppUserList}" var="users">
			...
			template += '<li><a href="javascript:void(0);" class="small"  tabIndex="-1"><input name="evaluationApprovals[' + (cnt - 1) + '].approvalType" checked="checked" value="OR" class="approval_condition" type="checkbox"/><label>Any</label></a></li>';
			template += '<li><a href="javascript:void(0);" class="small " tabIndex="-1"><input name="evaluationApprovals[' + (cnt - 1) + '].approvalType" value="AND" class="approval_condition"  type="checkbox"/><label>All</label></a></li>	</ul>';
		...
		$(document).delegate('.suspRemoveApproval', 'click', function(e) {
			...
			$(".new-suspApproval").each(function(i, v) {
				...
				$(this).find(".approval_id_hidden").each(function(){					
					$(this).attr("name",'evaluationApprovals[' +(i-1) + '].id');					
				});
				
				$(this).find(".approval_condition").each(function(){
					$(this).attr("name",'evaluationApprovals[' +(i-1) + '].approvalType');
				}) // checkbox name reindex
				
				$(this).find("input[name='_evaluationApprovals[" + i + "].approvalType']").each(function(){
					$(this).attr('name','_evaluationApprovals[' +(i-1) + '].approvalType');
				}); //Checkbox hidden val reindex
				
				
				$(this).find(".tagTypeMultiSelect").each(function(){
					$(this).attr("name",'evaluationApprovals[' +(i-1) + '].approvalUsers');
					$(this).attr("id", "suspensionMultipleSelect-" + ((i-1)) + "");
				}) //select name reindex
				
				$(this).find("input[name='_evaluationApprovals[" + i + "].approvalUsers']").each(function(){
					$(this).attr("name",'_evaluationApprovals[' +(i-1) + '].approvalUsers');
		...
		if($('#enablePerformanceEvaluationApproval').is(':checked')){
			$('#addAppLevel').removeClass('disabled');
		}else{
			$('#addAppLevel').addClass('disabled');
		}
		
		 $('#enablePerformanceEvaluationApproval').change(function() {
		     if(this.checked) {
		    	 $('#addAppLevel').removeClass('disabled');
		     }else {
		    	 $('#addAppLevel').addClass('disabled');
	...
	window.onload = function() {
		...
		document.getElementById("addAppLevel").addEventListener("click", function() {
